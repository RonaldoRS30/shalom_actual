...
class Caja extends Controller{
    ...
    #########################
        ...
        public function datatable_caja(){
            ...
            if (count($cajaInfo) > 0) {
				...
				foreach ($cajaInfo as $indice => $valor) {
					if ($valor->CAJA_FlagSituacion == '1') {
						$btn_modal = "<button type='button' class='btn btn-default' disabled
										onclick=\"alert('Caja activa no es posible editarla');\">
					...
					// Mensaje de estado
					$msgEstado = ($valor->CAJA_FlagSituacion == '0') 
								? "<span class='bold color-red' style='display: block; text-align: center;'>CERRADA</span>" 
		...
		public function guardar_registro(){
			...
			$filter->USUA_Codigo = $cajero_caja;
			$filter->CAJA_FlagSituacion = $estado_caja;//1 - Abierto,0 - Cerrada
			$filter->CAJA_Usuario = $this->usuario;
