...
class Cajacierre extends controller{
    ...
    public function datatable_cajacierre(){
		...
		if (count($cierres_caja) > 0) {
			foreach ($cierres_caja as $indice => $valor) {
				...
				$btn_modal = "<button type='button' onclick='modal_movimientoscierre($valor->CAJCIERRE_Codigo,\"$valor->CAJA_Nombre\",\"$valor->nombres\",\"$valor->CAJCIERRE_Ingresos\",\"$valor->CAJCIERRE_Egresos\",\"$valor->CAJCIERRE_Saldo\",\"$valor->CAJCIERRE_Fapertura\",\"$valor->CAJCIERRE_Fcierre\")' class='btn btn-default'>
								<img src='". base_url()."/images/icono-documentos.png' class='image-size-1b'>
							</button>";
	
				// Verificar si CAJCIERRE_Fcierre es nulo
				if (is_null($valor->CAJCIERRE_Fcierre) || $valor->CAJCIERRE_Fcierre == "") {
					$btn_cierre = "<button type='button' onclick='closecaja($valor->CAJCIERRE_Codigo)' class='btn btn-default' title='CERRAR CAJA'>
									<img src='". base_url()."/images/caja.jpg' class='image-size-1b'>
								</button>";
				} else {
					$btn_cierre = ""; // No mostrar el botón si CAJCIERRE_Fcierre no es nulo
				}
	...
	public function Closecaja(){
		...
		$idcierre = $this->input->post("id");
		$flagC = $this->input->post("flagC");

		$info = new stdClass();
		$info->CAJCIERRE_FlagSituacion = $flagC;
		$date = date("Y-m-d H:i:s");

		$info->CAJCIERRE_Fcierre = $date;

		$send_update= $this->cajacierre_model->actualizar_cierre($idcierre ,$info);

		/* Aca traemos los datos del cierre para conseguir el ID de la caja asiganda a este cierre */
		$datosCierre = $this->cajacierre_model->getCierre($idcierre);
		
		$id_caja = $datosCierre[0]->CAJA_Codigo;

		if($send_update){
			$situacion_caja = new stdClass();
			$situacion_caja->CAJA_FlagSituacion = $flagC;
			$this->caja_model->actualizar_caja($id_caja, $situacion_caja);
    ...
    public function datatable_movimientoscierre(){
		...
		$this->session->set_userdata('caja_codigo', $caja);
        ...
        $movimientosInfo = $this->movimiento_model->getMovimientosC($filter);
        ...
        if (count($movimientosInfo) > 0) {
            foreach ($movimientosInfo as $indice => $valor) {
				...
				// Determinar el color y texto del movimiento
				if (isset($valor->CPC_FlagEstado) && $valor->CPC_FlagEstado == 0) {
					$movimiento = "<span class='bold' style='color: red;'>ANULADO</span>";
				} else {
					// Si el documento no está anulado, verificar tipo de movimiento
					$colors     = ($valor->CAJAMOV_MovDinero == 2) ? "color-red" : "color-green";
					$movimiento = "<span class='bold $colors'>".$valor->movimiento."</span>";
					
					// Ajustar ingresos o egresos según el tipo de movimiento
        ...
        $json = array(
                    "draw"            => intval( $this->input->post('draw') ),
                    "recordsTotal"    => count($this->movimiento_model->getMovimientosC()),
                    "recordsFiltered" => intval( count($this->movimiento_model->getMovimientosC($filter)) ),
                    "data"            => $lista,
                    "totales"         => array(
                                            "ingresos" => $ingresos,
                                            "egresos"  => $egresos,
