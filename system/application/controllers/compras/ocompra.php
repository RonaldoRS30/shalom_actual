...
class Ocompra extends Controller{
  ...
  public function ocompras($tipo_oper = 'C', $param = NULL){
    ...
    $data['producto'] = $this->pedido_model->pedidos_diarios();
    ...
    if ($error == false){
	    ...
	    if ($detalle != NULL) {
	      foreach ($detalle as $indice => $valor) {
	        ...
	        $filterGuia->GUIAREMDETC_Peso = $valor->OCOMP_Peso;
  ...
  ## Dev: RC -> End
    ...
    public function nueva_ocompra($tipo_oper = 'C',$sucursal=''){
        ...
        $data['cboFormapago'] = form_dropdown("formapago", $this->formapago_model->seleccionar(), "23", " class='comboMedio' id='formapago'");
    ...
    public function ocompra_pedido($proveedor, $tipo_oper = 'C', $sucursal, $productos, $cantidad){
        $hoy = date('Y-m-d H:i:s');
        $cadena = strtotime($hoy).substr((string)microtime(), 1, 8);
        $tempSession = str_replace('.','',$cadena);
        $data['tempSession']  = $tempSession;
        $proveedorData = $this->proveedor_model->getProveedor($proveedor);
        $productosCodigos = explode("-",$productos);
        $cantidadProductos = explode("-",$cantidad);
        ...
        $data['CodProductos'] = $productosCodigos;
        $data['CantProductos'] = $cantidadProductos;
        ...
        $data['nombre_proveedor'] = $proveedorData[0]->razon_social;
        $data['ruc_proveedor'] = $proveedorData[0]->numero;
        $data['pendiente'] = "";
        $data['ctactesoles'] = "";
        $data['ctactedolares'] = "";
        $data['preciototal'] = "";
        ...
        if (is_array($prodcodigo)) {
            foreach ($prodcodigo as $indice => $valor) {
                ...
                if($accion != "e" && $accion != "EE"){
                    ...
                    $filter->OCOMP_Peso = $prodpeso[$indice];
