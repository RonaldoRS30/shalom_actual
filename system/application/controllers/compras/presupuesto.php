...
class Presupuesto extends Controller {
    ...
    public function presupuestos($j = '0', $limpia = '') {
        ...
        $conf['base_url'] = site_url('compras/presupuesto/presupuestos');
        $conf['per_page'] = 12;
        $conf['num_links'] = 3;
        $conf['first_link'] = "&lt;&lt;";
        $conf['last_link'] = "&gt;&gt;";
        ...
        $data['titulo_busqueda'] = "BUSCAR COTIZACIONES";
        $data['lista'] = $lista;
        $data['oculto'] = form_hidden(array('base_url' => base_url()));
        $data['tipo_codificacion'] = $data_confi_docu[0]->COMPCONFIDOCP_Tipo;
        $this->pagination->initialize($conf);
        $data['paginacion'] = $this->pagination->create_links();
        $this->layout->view('compras/presupuesto_index', $data);
    }
    ...
    public function presupuesto_nueva($tipo_docu) {
        ...
        $data['url_action'] = base_url() . "index.php/compras/presupuesto/presupuesto_insertar";
        $data['titulo'] = "REGISTRAR COTIZACION";
		...
		if($numero_predt!=null && $numero_predt!=0 && count($numero_predt)>0)
        	$numero = $numero_predt[0]->PRESUC_Numero + 1;
        else
        	$numero =1;
        ...
        $data['numero_suger'] = $this->presupuesto_model->obtener_ultimo_numero($serie);

        $this->layout->view('compras/presupuesto_nuevo', $data);
    }
    ...
    public function presupuesto_editar($codigo) {
        ...
        $data['cboPedidos'] = form_dropdown("pedidos", $this->pedido_model->seleccionar(), $pedido, " onchange='load_cotizaciones();' class='comboGrande' style='width:200px;' id='pedidos'");
        ...
        $data['contiene_igv'] = (($data_confi[0]->COMPCONFIC_PrecioContieneIgv == '1') ? true : false);
        $oculto = form_hidden(array('codigo' => $codigo, 'base_url' => base_url(), 'tipo_docu' => $tipo_docu, 'contiene_igv' => ($data['contiene_igv'] == true ? '1' : '0'), "tipo_codificacion" => $data_confi_docu[0]->COMPCONFIDOCP_Tipo));
        $data['titulo'] = "EDITAR COTIZACION";
        $data['formulario'] = "frmCotizacion";
        $data['oculto'] = $oculto;
        $data['onload'] = "onload=\"\"";
        $data['url_action'] = base_url() . "index.php/compras/presupuesto/presupuesto_modificar";
        $atributos = array('width' => 700, 'height' => 450, 'scrollbars' => 'yes', 'status' => 'yes', 'resizable' => 'yes', 'screenx' => '0', 'screeny' => '0');
        $contenido = "<img height='16' width='16' src='" . base_url() . "images/ver.png' title='Buscar Cliente' border='0'>";
...
///GCBQ Ventana Presupuesto para comprobante segun tipo
	public function ventana_muestra_presupuestoCom($tipo_oper, $codigo = '', $formato = 'SELECT_ITEM', $tipo_doc , $almacen = "" , $comprobante ='',$ventana='') {
		...
		$lista_comprobante = $this->presupuesto_model->buscar_presupuestos($filter);
	 ...
	 ///GCBQ Ventana Cotizacion Recurrente
	public function ventana_muestra_presupuestoRecu($tipo_oper, $codigo = '', $formato = 'SELECT_ITEM', $docu_orig ='G', $almacen = "" , $comprobante ='') {
	  $data['compania'] =$this->somevar['compania'];
        ...
        foreach ($lista_comprobante as $indice => $value) {
            ...
            $select = '';
            if ($formato == 'SELECT_HEADER')
                $select = "<a href='javascript:;' onclick='seleccionar_cotizacion(" . $value->PRESUP_Codigo ." ," .$value->PRESUC_Serie.",".$value->PRESUC_Numero.")'><img src='" . base_url() . "images/ir.png' width='16' height='16' border='0' title='Seleccionar Comprobante'></a>";
            $lista[] = array(mysql_to_human($value->PRESUC_Fecha), $value->PRESUC_Serie, $value->PRESUC_Numero, '', $value->nombre, $value->MONED_Simbolo . ' ' . number_format($value->PRESUC_total), $ver, $select);
        }
        ...
        $data['formato'] = $formato;
        $data['form_open'] = form_open(base_url() . "index.php/compras/ventana_muestra_cotizacion", array("name" => "frmComprobante", "id" => "frmComprobante"));
        $data['form_close'] = form_close();
       $data['form_hidden'] = form_hidden(array("base_url" => base_url(), "docu_orig" => $docu_orig, "formato" => $formato));

        $this->load->view('compras/ventana_muestra_cotizacion', $data);
	 } 
    ...
    public function presupuesto_ver_pdf($codigo) {
        switch (FORMATO_IMPRESION) {
            ...
            case 4:  //Formato para ferremax
                ...
                break;
//            default: presupuesto_ver_pdf_conmenbrete_formato1($codigo);
//                break;
            default: presupuesto_ver_pdf_formato1($codigo);
                break;
        }
    }
    ...
    public function presupuesto_ver_xls($codigo) {
        ...
        $xls = utf8_decode_seguro('<b>Cotización: ') . $codificacion . '</b>';
        ...
        $xls .= "
		<table border=1>
			<tr><th>Item</th><th>Marca</th><th>" . utf8_decode_seguro('Descripción') . "</th><th>Uni.</th><th>Cant.</th><th>Precio Uni.</th><th>Precio Total</th></tr>
		";
        ...
        $xls .= "
			...
			<tr><td>Los Precios de los Productos</td><td>" . ($modo_impresion == '1' ? 'CONTIENEN IGV' : 'NO CONTIENEN IGV') . "</td></tr>
			...
			<tr><td>Tiempo de Entrega</td><td>$tiempo_entrega</td></tr>
			<tr><td>Lugar de Entrega</td><td>" . utf8_decode_seguro($lugar_entrega) . "</td></tr>
			<tr><td>Validez de la Oferta</td><td>" . utf8_decode_seguro($validez) . "</td></tr>
...
//echo "<br/>".$empresa;
        ...
        $temp = $this->obtener_datos_cliente($proveedor, $tipo_docu);
        $nombre_cliente = $temp['nombre'];
        $ruc = $temp['numdoc'];
        $direccion = $temp['direccion'];
        // $direccion = "";
        ...
        $this->cezpdf->ezText(utf8_decode_seguro('<b>Cotización de Compra: ') . $codificacion . '</b>', 17, array("leading" => 40, 'left' => 185));
    ...
    public function presupuesto_ver_pdf_conmenbrete_formato1($codigo) {
        ...
        //prep_pdf();
        //$this->cezpdf = new Cezpdf('a4');
        //$this->cezpdf = new backgroundPDF('a4', 'portrait', 'image', array('img' => 'images/img_db/logo_instrume_unico.jpg'));
        $datacreator = array(
            'Title' => 'Estadillo de ',
            'Name' => 'Estadillo de ',
            'Author' => 'Vicente Producciones',
        ...
        $this->cezpdf = new Cezpdf('a4', 'landscape');
        $delta = 20;
        //$this->cezpdf->ezImage("images/img_db/logo_instrume_unido.jpg", 20, 740, 'none', 'left');
    ...
    public function obtener_detalle_presupuesto($presupuesto) {
        ...
        $proveedor_llamar = $datos_presupuesto[0]->PROVP_Codigo;
        $temp = $this->obtener_datos_cliente($proveedor_llamar);
        $ruc = $temp['numdoc'];
    ...
    function obtener_datos_cliente($proveedor, $tipo_docu = 'F') {
...
//        $tipo = $datos_proveedor[0]->CLIC_TipoPersona;
        ...
        $nombre = $datos_proveedor[0]->EMPRC_RazonSocial;
        $numdoc = $datos_proveedor[0]->EMPRC_Ruc;
        ...
        $telefono = $datos_proveedor[0]->EMPRC_Telefono;
        $movil = $datos_proveedor[0]->EMPRC_Movil;
        $fax = $datos_proveedor[0]->EMPRC_Fax;
        $email = $datos_proveedor[0]->EMPRC_Email;
        $contacto = '';
