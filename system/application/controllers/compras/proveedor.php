...
class Proveedor extends Controller{
	...
	public function search_documento(){
		...
		// Cambiamos documento_exists para contar los registros en lugar de solo verificar existencia
		$count = $this->empresa_model->documento_exists($numero, $empresa_pertenece);
	
		if ($count < 2){ // Si hay menos de 2 registros, permitir el registro
			...
			if (is_numeric($numero)){
				...
				if ($object->match == true){
					...
					$json = array(
						"exists" => ($count > 0), // Existe si hay al menos un registro
						"match" => true,
					...
					$json = array("exists" => ($count > 0), "match" => false, "message" => "Documento no encontrado.");
				}
			}
			else{
				$json = array("exists" => ($count > 0), "match" => false, "message" => "Formato de RUC/DNI invalido.");
			}
		}
		else{
			$json = array("exists" => true, "match" => true, "message" => "El documento $numero ya tiene 2 registros, no se puede agregar más.");
		}
		echo json_encode($json);
		die();
	}
