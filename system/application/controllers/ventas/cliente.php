...
class Cliente extends Controller{
  ...
  //Obtiene campos especificos de una empresa y lo renombra
		...
		if($datosCliente != NULL){
			...
			foreach ($datosCliente  as $key => $value) {
				...
				if ( $tipoPersona== '0') {
					...
					$sucursales = $this->empresa_model->listar_sucursalesEmpresa($codigoEmpresa);
					foreach($sucursales as $i => $val){
						$direccion[$i] = $val->EESTAC_Direccion;
					}
					$tipoDocIdentidad = $value->tipo_documento;
				}
	...
	public function autocomplete_ruc(){
		...
		if ($consulta != NULL) {
			...
			foreach ($consulta AS $cliente => $value) {
				...
				$vendedor = $value->CLIC_Vendedor;
				$digemin = $value->CLIC_Digemin;
				$result[] = array("value" => $ruc, "label" => "$value->CLIC_CodigoUsuario | $ruc - $nombre", "nombre" => $nombre, "codigo" => $value->CLIP_Codigo, "ruc" => $ruc, "TIPCLIP_Codigo" => $value->TIPCLIP_Codigo, "tipoPersona" => $tipoPersona, "codigoEmpresa" => $codigoEmpresa, "vendedor" => $vendedor, "contactos" =>  $contactos, "ubigeo" => $ubigeo, "digemin" =>  $digemin, "direccion" => $direccion, "tipoDocIdentidad" => $tipoDocIdentidad);
			}
		}
		echo json_encode($result);
	}
	...
	public function insertarClientesMasiva(){
        ...
        for ($row = 2; $row <= $highestRow; ++$row) {
          ...
          $clientes_repetidos = $sheet->getCellByColumnAndRow(1, $row)->getValue();
		  $getCliente = $this->cliente_model->getClientexDocu($clientes_repetidos);
    
    
          if ($getCliente == null) {
    
            $razon_social = $sheet->getCellByColumnAndRow(0, $row)->getValue();
            $documento = $sheet->getCellByColumnAndRow(1, $row)->getValue();
			$telefono = $sheet->getCellByColumnAndRow(2, $row)->getValue();
            $correo = $sheet->getCellByColumnAndRow(3, $row)->getValue();
            $direccion = $sheet->getCellByColumnAndRow(4, $row)->getValue();
            $vendedor = 1;
       

			if(strlen($documento) == 8 || strlen($documento) == 7){
				$tipo_cliente = 0;
				$nombresArr = explode(" ", $razon_social);
				$lenght = sizeof($nombresArr);
				$lenght2 = $lenght - 1;
				for($i = 2; $i > 0; $i--){
					$apellido_materno = $nombresArr[$lenght2];
					$apellido_paterno = $nombresArr[$lenght2-1];
				}

				for($i = 0; $i<$lenght; $i++){
					if($nombresArr[$i] != $apellido_materno){
						if($nombresArr[$i] != $apellido_paterno){
							$nombres.= $nombresArr[$i]." ";
						}
					}
				}
			}else{
			...
			$empresaInfo = new stdClass();
			$empresaInfo->CIIUP_Codigo      	= 0;
			$empresaInfo->TIPCOD_Codigo     	= $tipo_codigo;
			$empresaInfo->SECCOMP_Codigo    	= "";
			$empresaInfo->EMPRC_Ruc         	= $documento;
			$empresaInfo->EMPRC_RazonSocial 	= $razon_social;
			$empresaInfo->EMPRC_Telefono    	= "";
			$empresaInfo->EMPRC_Movil       	= $telefono;
			$empresaInfo->EMPRC_Fax         	= "";
			$empresaInfo->EMPRC_Web         	= "";
			$empresaInfo->EMPRC_Email       	= $correo;
			$empresaInfo->EMPRC_CtaCteSoles 	= "";
			$empresaInfo->EMPRC_CtaCteDolares	= "";
			$empresaInfo->EMPRC_FlagEstado  	= "1";
    	...
    	## PERSONA
			...
			$personaInfo->NACP_Nacionalidad     	= "0";
			$personaInfo->PERSC_Nombre          	= $nombres;
			$personaInfo->PERSC_ApellidoPaterno 	= $apellido_paterno;
			$personaInfo->PERSC_ApellidoMaterno 	= $apellido_materno;
			$personaInfo->PERSC_TipoDocIdentidad 	= 1;
			$personaInfo->PERSC_Ruc             	= "";
			$personaInfo->PERSC_NumeroDocIdentidad 	= $documento;
			$personaInfo->PERSC_FechaNac        	= "0000-00-00";
			$personaInfo->PERSC_FechaNacz        	= "0000-00-00";
			$personaInfo->PERSC_Direccion       	= $direccion;
			$personaInfo->PERSC_Telefono        	= "";
			$personaInfo->PERSC_Movil           	= $telefono;
			$personaInfo->PERSC_Fax             	= "";
			$personaInfo->PERSC_Email           	= $correo;
			$personaInfo->PERSC_Domicilio       	= $direccion;
			$personaInfo->PERSC_Web             	= "";
			$personaInfo->PERSC_FlagEstado      	= "1";
			$personaInfo->PERSC_EmpCod      		= $empresa_pertenece;
		...
		## CLIENTE
			$clienteInfo = new stdClass();
			$clienteInfo->CLIC_TipoPersona   = $tipo_cliente;
			$clienteInfo->TIPCLIP_Codigo     = 1;
			$clienteInfo->CLIC_Vendedor      = $vendedor;
			$clienteInfo->FORPAP_Codigo      = 1;
			# SOLO EN LAZPER, este campo guarda el estado de la empresa en digemid
			$clienteInfo->CLIC_Digemin       = "";
			$clienteInfo->CLIC_flagCalifica  = 1;
			$clienteInfo->CLIC_FlagEstado    = "1";
		...
		## SUCURSAL
			$sucursalInfo = new stdClass();
			$sucursalInfo->TESTP_Codigo = 1;
			$sucursalInfo->UBIGP_Codigo = "000000";
			$sucursalInfo->EESTABC_Descripcion = "PRINCIPAL";
			$sucursalInfo->EESTAC_Direccion = $direccion;
			$sucursalInfo->EESTABC_FlagTipo = "1";
			$sucursalInfo->EESTABC_FlagEstado = "1";

			$result = $this->empresa_model->EmpresaExiste($documento, $tipo_cliente, $this->empresa);
			...
			if ( $es_cliente !=null){
				$json_result = "error";
				$json_message = "El número de documento $documento, ya se encuentra registrado.";
			}
			else{
				if ($es_cliente == null && $es_proveed == null) {
					if ($tipo_cliente == "0"){
              ...
              foreach ($clientes_repetidos as $value) {
                  $array_datos = array(
                      "codigo_interno_producto" => $value->CLIP_Codigo,
                      "descripcion_codigo" => $value->razon_social
                  );
	...
	public function getFormatoCargaClientes(){
        ...
        /** Columnas **/
        $lugar = 1;
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("A$lugar",  "NOMBRE DE CLIENTE"); 
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("B$lugar",  "DOCUMENTO");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("C$lugar",  "TELEFONO");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("D$lugar",  "CORREO");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("E$lugar",  "DIRECCION");
    
        $this->excel->getActiveSheet()->getStyle("A$lugar:G$lugar")->applyFromArray($estiloTitulo);
        $this->excel->getActiveSheet()->getRowDimension($lugar)->setRowHeight(30);
        $lugar++;
        
        /** Items **/
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("A$lugar",  "ccapasistemas");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("B$lugar",  "20523262816");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("C$lugar",  "981005695");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("D$lugar",  "sistemas@ccapasistemas.com");
        $this->excel->setActiveSheetIndex($hoja)->setCellValue("E$lugar",  "LINCE Manuel Segura 122 Oficina 801 (Alt 15 de Arenales)");
    
        $filename = "Formato de carga Clientes.xls";
