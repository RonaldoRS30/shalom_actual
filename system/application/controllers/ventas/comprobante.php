...
class Comprobante extends Controller{
    ...
    # INICIO
        ...
        public function datatable_comprobantes($tipo_oper = '', $tipo_docu = ''){
            ...
            if ($listado_comprobantes != NULL) {
                foreach ($listado_comprobantes as $indice => $valor) {
                    ...
                    $listaPedidoGuiaAsociado = $this->pedido_model->buscarPedidoGuiarem($valor->PEDIP_Codigo, $estadoAsociacion);
                    $guiarem_codigo         = '';
                    ...
                    if ( count($listaGuiaremAsociados) > 0 ){
                        ...
                        if ( count($listaPedidoGuiaAsociado) > 0 ){
                            foreach ( $listaPedidoGuiaAsociado as $j => $valorGuiarem ){
                                $codigoGuiarem   = $valorGuiarem->GUIAREMP_Codigo;
                                $serieG          = $valorGuiarem->GUIAREMC_Serie;
                                $numeroG         = $valorGuiarem->GUIAREMC_Numero;
                                $estadoRelacion  = $valorGuiarem->COMPGUI_FlagEstado;
                ...
                if ($aceptada["exito"] == true || $comprobante->CPC_TipoDocumento == 'N') {
                    ...
                    if ($comprobante->CPC_FlagMueveStock == '0') {
                        ...
                        if($this->db->trans_status() == false) {
                            ...
                            $movimiento = ($comprobante->CPC_TipoOperacion == 'V') ? $this->disminuir_stock($comprobante) : $this->aumentar_stock($comprobante);
                            $this->stockComprometido($codigo);
                            ...
                            /* script cierre de ccaja cuadre de monto */
                            if($comprobante->CAJA_Codigo != "" || $comprobante->CAJA_Codigo != 0 || $comprobante->CAJA_Codigo != null){
                            ...
                            if ($tipo_oper == "V") {
                                ...
                                $monto_a_sumar = $comprobante->CPC_total;
                                ...
                                $this->cajacierre_model->actualizar_cierre($id_cierre, $ingresos);
                                ...
                                $this->cajacierre_model->actualizar_cierre($id_cierre, $egresos);
                            }
        ...
        public function disparador_pos($tipo_oper = 'V', $codigo, $tipo_docu = 'F'){
            ...
            if ($comprobante->CPC_FlagEstado == '2'){
                ...
                if ($aceptada["exito"] == true || $comprobante->CPC_TipoDocumento == 'N'){
                    ...
                    if ($comprobante->CPC_FlagMueveStock == '0') {
                        $this->db->trans_start();
                        $query = $this->db->query("CALL COMPROBANTE_DISPARADOR(?)", array($codigo)); // Use query binding
        ...
        public function comprobante_eliminar(){
            ...
            $infoComprobante = $this->comprobante_model->obtener_comprobante($comprobante);
            $tipo_oper = $infoComprobante[0]->CPC_TipoOperacion;
            $id_caja = $infoComprobante[0]->CAJA_Codigo;
            $id_cierre = $infoComprobante[0]->CAJCIERRE_Codigo;
           
            $ultima_Caja = $this->cajacierre_model->getCierre($id_cierre);

            if ($tipo_oper == "V") {
                $monto_actual_ingresos = $ultima_Caja[0]->CAJCIERRE_Ingresos;
                $monto_a_sumar = $infoComprobante[0]->CPC_total;
                $total_ingresos = $monto_actual_ingresos - $monto_a_sumar;
            ...
            // Obtener los ingresos y egresos actualizados
            $actualizado_Caja = $this->cajacierre_model->getCierre($id_cierre);
            $total_ingresos = $actualizado_Caja[0]->CAJCIERRE_Ingresos;
            $total_egresos = $actualizado_Caja[0]->CAJCIERRE_Egresos;
    ...
    public function guardar_pago_pend(){
        $comprobante = $this->input->post("comprobante_id");
        $fomra_pago = $this->input->post("nvopago_formaPago");
        $caja = $this->input->post("nvopago_caja");
        $nvopago_moneda = $this->input->post("nvopago_moneda");
        $monto = $this->input->post("nvopago_monto");
    
        $comprobanteInfo = $this->comprobante_model->obtener_comprobante($comprobante);
        $comprobante = $comprobanteInfo[0];
        
        $getCuenta = $this->cuentas_model->getCuenta_pend($comprobante->CPP_Codigo);
          ...
          $filter->PAGC_FormaPago = $fomra_pago; # POR AHORA EFECTIVO
        ...
        # (V:1) = INGRESO | (C:2) = EGRESO
           ...
           /*    $stdFormasPago                  = new StdClass();
              $stdFormasPago->CPP_Codigo      = $comprobante->CPP_Codigo;
              $stdFormasPago->FORPAP_Codigo   = $fomra_pago;
              $stdFormasPago->MONED_Codigo    = $comprobante->MONED_Codigo;
              $stdFormasPago->monto           = $monto;
              $stdFormasPago->compro_flag_FechaRegistro = date('Y-m-d h:i:s');
         
             $result = $this->comprobante_formapago_model->insertar($stdFormasPago);
       */
    ...
    public function comprobante_insertar(){
        ...
        $invcodigo      = $this->input->post('inter_fac');
        if($invcodigo == "on"){
            $invcodigo = 1;
        }else{
            $invcodigo = 0;
        }
        $pais_cod       = $this->input->post('pais_cod');
        ...
        $filter->CPC_Tipclip    = $this->input->post('TipCli');
        ...
        if($tipo_oper == 'C')
            ...
            $filter->INV_FlagEstado = $invcodigo;
            if($invcodigo == 1){
                $filter->PAIS_Codigo = $pais_cod;
            }else{
        ...
        $proddescuentos100 = $this->input->post('descuento');
        ...
        if (is_array($prodcodigo)) {
            foreach ($prodcodigo as $indice => $valor) {
                ...
                $filter->CPDEC_Descuento100 = $proddescuentos100;
    ...
    public function insertar_guiarem($comprobante){
        ...
        $comprobantePedido = $this->comprobantepedido_model->listarComprobante($comprobante);
        if($comprobantePedido != NULL){
            $pedido = $comprobantePedido[0]->PEDIP_Codigo;
            $guiaPedido = $this->guiarempedido_model->listarPedido($pedido);
            if($guiaPedido != NULL){
                $guia_ant = $guiaPedido[0]->GUIAREMP_Codigo;
                $sernum_ant = $guiaPedido[0]->GUIAREMC_Serie."-".$guiaPedido[0]->GUIAREMC_Numero;
    ...
    public function stockComprometido($codigo){
        $comprobantePedido = $this->comprobantepedido_model->listarComprobante($codigo);
        if (count($comprobantePedido) > 0) {
            $detalle_comprobante = $this->comprobantedetalle_model->detalles($codigo);
            foreach ($detalle_comprobante as $indice => $valor) {
                $producto = $valor->PROD_Codigo;
                $almacen = $valor->ALMAP_Codigo;
                $cantidad = $valor->CPDEC_Cantidad;
                $stockComprometido = $this->almacenproducto_model->AprobarPedido($almacen, $producto, $cantidad);
            }
        }
    }
    ...
    public function comprobante_editar($codigo, $tipo_oper = 'V', $tipo_docu = 'F'){
        ...
        $pais               = $datos_comprobante[0]->PAIS_Codigo;
        $paisnombre         = $datos_comprobante[0]->pais_nombre;
        $paisiso            = $datos_comprobante[0]->pais_iso;
        $invoice            = $datos_comprobante[0]->INV_FlagEstado;
        $categoriaprecioped = $datos_comprobante[0]->CPC_Tipclip;
        ...
        $data['pais']      = $pais;
        $data['paisnombre']= $paisnombre;
        $data['paisiso']   = $paisiso;
    ...
    public function comprobante_modificar(){
        ...
        $prodigv100         = $this->input->post('prodigv100');
        $proddescuento100   = $this->input->post('descuento');
        $prodcosto          = $this->input->post('prodcosto');
        $almacenProducto    = $this->input->post('almacenProducto');
        $proddescri         = $this->input->post('proddescri');
        $obs_detalle        = $this->input->post('prodobservacion');
        ...
        if (is_array($detacodi) > 0) {
            foreach ($detacodi as $indice => $valor) {
                ...
                $filter->CPDEC_Total = $prodimporte[$indice];
                $filter->CPDEC_Pu_ConIgv = $prodpu_conigv[$indice];
                $filter->CPDEC_Descuento100 = $proddescuento100;
                $filter->CPDEC_Igv100 = $prodigv100[$indice];
                if ($tipo_oper == 'V')
                    $filter->CPDEC_Costo = $prodcosto[$indice];
   ...
   /**gcbq json verificacion de cantidad por producto y cantidad por serie este metodo lo usa json de inventario guiarem y otros***/
    ...
    public function getDetalle($codigo){
        $data = $this->comprobantedetalle_model->detalles($codigo);
    ...
    public function autocompletePais(){
		...
		$datosPais = $this->comprobante_model->autocompletePais($keyword);
		$result = array();

		if($datosPais != NULL){
			foreach ($datosPais  as $key => $value) {
                $nombre = $value->PAIS_Descripcion;
                $iso    = $value->PAIS_Iso;
                $codigo = $value->PAIS_Codigo;
				$result[] = array("value" => $nombre, "label" => $nombre,"nombre" => $nombre, "iso" => $iso, "codigo" => $codigo);
			}
		}
		echo json_encode($result);
    }
