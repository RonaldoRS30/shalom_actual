...
/*"<a href='javascript:;' onclick='comprobante_ver_pdf_conmenbrete(" . $codigo . ")' target='_parent'><img src='" . base_url() . "images/pdf.png' width='16' height='16' border='0' title='Ver PDF'></a>";*/
     ...
     public function comprobantes($tipo_oper = 'V', $tipo_docu = 'F', $j = '0')
    {
     ...
     //   $tipo_oper = $this->uri->segment(4);
      //  $tipo_docu = $this->uri->segment(5);
        ...
        $data['titulo_tabla'] = "RELACIÓN DE " . strtoupper($this->obtener_tipo_documento("F")) . "IMPORTACIONES";
        $data['titulo_busqueda'] = "BUSCAR " . strtoupper($this->obtener_tipo_documento("F"));
        $data['tipo_oper'] = $tipo_oper;
        $data['tipo_docu'] = "I";
        $data['oculto'] = form_hidden(array('base_url' => base_url(), 'tipo_oper' => $tipo_oper, "tipo_docu" => "I"));
    ...
    public function datatable_importacion($tipo_oper = '', $tipo_docu = ''){
        $j = 0;
        $posDT = -1;
        $filter = new stdClass();
        ...
        $listado_comprobantes = $this->importacion_model->buscar_comprobantes($tipo_oper, $tipo_docu,$filter);
        $item = $j + 1;
        $contadoVacios = 1;
        $lista = array();
        if (count($listado_comprobantes) > 0) {
            foreach ($listado_comprobantes as $indice => $valor) {
                ...
                $img_estado = ($estado == '1' || $estado == '2' ? $img_estado = '<a onclick="abrirAnulacionModal('.$codigo.',\''.$serie.'\',\''.$numero.'\',\''.$total.'\');" href="javascript:;" class="enviarcorreo"><img src="' . base_url() . 'images/active.png" width="16" height="16" border="0" title="Anular documento"></a>' : "<img src='" . base_url() . "images/inactive.png' alt='Anulado' title='Anulado' />");
               // $impor_liquidada=$valor->;
                ...
                $excel = "<a href='javascript:;' onclick='importacion_excel(".$codigo.")'><img src='" . base_url() . "images/excel.png' width='16' height='16' border='0' title='Ver PDF'></a>";
                $dua = $valor->DUA_Codigo;
                if($estado == "2"){
                    $aprobar = "<a href='javascript:;' onclick='disparador($codigo)'>Aprobar</a><br>";
                }else{
                    $aprobar = "";
                    $editar = "<img src='" . base_url() . "images/complete.png' width='16' height='16' border='0' title='Aprobado'>";
                }
                $lista[] = array(
                    $fecha, 
                    $serie,
                    $this->getOrderNumeroSerie($numero),
                    $dua,
                    $nombre,
                    $total, 
                    $editar, 
        ...
        $recordsTotal = $this->importacion_model->buscar_comprobantes($tipo_oper, $tipo_docu,$filterAll);
        $recordsFiltered = $this->importacion_model->buscar_comprobantes($tipo_oper, $tipo_docu,$filter);
        
        $json = array(
            "draw"            => intval( $this->input->post('draw') ),
            "recordsTotal"    => $recordsTotal[0]->registros,
            "recordsFiltered" => $recordsFiltered[0]->registros,
            "data"            => $lista
        );
        
        echo json_encode($json);
    }
    public function importacion_nueva($tipo_oper = 'V', $tipo_docu = 'I')
    {
       // $tipo_oper = $this->uri->segment(4);
       // $tipo_docu = $this->uri->segment(5);
      //  $tipoDocumento = strtoupper($this->obtener_tipo_documento($tipo_docu));
       ...
       // }else {
            ...
            $data['url_action'] = base_url() . "index.php/ventas/importacion/importacion_insertar";
...
public function select_cmbVendedor($index){
    ...
    public function importacion_insertar(){
        ...
        $serie = $this->input->post('serie');
        $numero = $this->input->post('numero');
        $duacod = $this->input->post('dua_cod');
        $adValorem = $this->input->post('adValorem');
        ...
        $filter->DUA_Codigo = $duacod;
        $filter->IMPOR_advalorem100 = $adValorem;
      ...
      //  }
        ...
        $filter->IMPOR_Fecha = $this->input->post('fecha');
        ...
        if ($tipo_docu != 'B' && $tipo_docu != 'N') {
            $filter->IMPOR_cif = $this->input->post('importetotal');
        ...
        $filter->IMPOR_advalorem = $this->input->post('advaloremspan');
        $filter->IMPOR_total     = $this->input->post('totalimport');
        ...
        $servcodigo     = $this->input->post('servcodigo');
        $servnom        = $this->input->post('servnom');
        $servcant       = $this->input->post('servcant');
        $servpu         = $this->input->post('servpu');
        $servpuigv      = $this->input->post('servpuigv');
        $servtotal      = $this->input->post('servtotal');
        $servigv        = $this->input->post('servigv');
        $servsubtotal   = $this->input->post('servsubtotal');
        $servigv100     = $this->input->post('servigv100');

        if(is_array($servcodigo)){
            foreach($servcodigo as $cont => $val){
                $filterServ = new stdClass();
                $filterServ->IMPOR_Codigo           = $comprobante;
                $filterServ->PROD_Codigo            = $servcodigo[$cont];
                $filterServ->IMPSER_Cantidad        = $servcant[$cont];
                $filterServ->IMPSER_Pu              = $servpu[$cont];
                $filterServ->IMPSER_PuIGV           = $servpuigv[$cont];
                $filterServ->IMPSER_Igv100          = 18;
                $filterServ->IMPSER_Subtotal        = $servsubtotal[$cont];
                $filterServ->IMPSER_Igv             = $servigv[$cont];
                $filterServ->IMPSER_Total           = $servtotal[$cont];
                $filterServ->IMPSER_Descripcion     = $servnom[$cont];
                
                $this->importacion_model->insertar_imposerv($filterServ);
            }
        }

        $codi   = $this->input->post('prodservCodi');
        $prodservCodi   = $this->input->post('servCodi');
        $prodservTotal  = $this->input->post('prodservTotal');
        $adValorem  = $this->input->post('adValorem');
        ...
        if (is_array($prodcodigo)) {
            foreach ($prodcodigo as $indice => $valor) {
                ...
                if ($detaccion[$indice] != 'e' && $detaccion[$indice] != 'EE') {
                    $detaid = $this->importaciondetalle_model->insertar($filter);

                    if(is_array($codi)){
                        foreach($codi as $count => $val){
                            foreach($val as $k => $codiS){
                                if($codiS == $prodcodigo[$indice]){
                                    $filterServPro = new stdClass();
                                    $filterServPro->IMPOR_Codigo    = $comprobante;
                                    $filterServPro->IMPORDEP_Codigo = $detaid;
                                    $filterServPro->PROD_Codigo     = $prodservCodi[$indice][$k];
                                    $filterServPro->PROCOST_Costeo  = $prodservTotal[$indice][$k];
                                    $servprodeta = $this->importacion_model->insertardetaServ($filterServPro);
                                }
                            }
...
//	
    ...
    {   
        ...
        $fecha = $datos_comprobante[0]->IMPOR_Fecha;
        ...
        $dua        = $datos_comprobante[0]->DUA_Codigo;
        ...
        $data['adValorem'] = $datos_comprobante[0]->IMPOR_advalorem100;
...
//---------------------------------------
    ...
    public function importacion_modificar()
    {
        if ($this->input->post('serie') == '')
            exit('{"result":"error", "campo":"serie"}');
        if ($this->input->post('numero') == '')
...
//$tipo_docu = $datos_presupuesto[0]->PRESUC_TipoDocumento;
    ...
    public function importacion_eliminar()
    {
        $importacion = $this->input->post('importacion');
        $data = $this->importacion_model->anularImportacion($importacion);
        if($data){
            $data = array( 
                "estado"        => "success",
                "movimiento"    => 1,
                "mensaje"       => "La importación fue anulada"
            );
        }else{
            $data = array( 
                "estado"        => "error",
                "movimiento"    => 1,
                "mensaje"       => "Se ha presentado un error al anular la importación"
            );
        }
...
echo $resultado;
 ...
 public function encuentrax_producto() {
    ...
    public function get_dua($dua){
        $comprobante = $this->comprobante_model->getDuaCodigo($dua);
        echo json_encode($comprobante);
    }

    public function getServicios(){
        $impoCod = $this->input->post("codigo");
        $data = $this->importacion_model->servicios_importacion($impoCod);

        echo json_encode($data);
    }

    public function excel_importacion($codigo){
        $this->lib_props->excel_impor($codigo);
    }

    public function aprobar_import(){
        $impoCod = $this->input->post("codigo");
        $data = $this->importacion_model->aprobar_importacion($impoCod);

        if($data){
            $data = array( 
                "estado"        => "success",
                "movimiento"    => 1,
                "mensaje"       => "La importación fue aprobada"
            );
        }else{
            $data = array( 
                "estado"        => "error",
                "movimiento"    => 1,
                "mensaje"       => "Se ha presentado un error al aprobar la importación"
            );
        }
