    ...
    public function datatable_almacen_producto(){
        ...
        if ( count($listado) > 0 ){
            foreach ($listado as $indice => $valor) {
                ...
                $cantidadComprometida = "<span style='color:red'>".$valor->ALMPROD_StockComprometido."</span>";
                                
                $marca = $valor->MARCC_Descripcion;

                $lista[] = array($codigo_prod, $nombre_prod, $familia, $marca, $cantidad,$cantidadComprometida, $valor->UNDMED_Simbolo, $valor->ALMAC_Descripcion);
                $item++;
            }
        }
...
#REPORTES ALMACEN
    ...
    public function verReporteExcel_Det( $modelo = "" ){
        ...
        $this->excel->setActiveSheetIndex(0)->setCellValue("H$lugar", 'V. TOTAL SOLES');
