...
class Temporaldetalle extends Controller{
    ...
    public function agregarProductoPedido($codigo, $tempS, $cantidadPro){

        $dataprod = $this->producto_model->getProducto($codigo);
        $almaprod = $this->almacenproducto_model->obtener(1, $codigo);
        ...
        $descripcionProd    = $dataprod[0]->PROD_Nombre;
        $costo              = $dataprod[0]->PROD_CostoPromedio;
        $stock              = $almaprod[0]->ALMPROD_Stock;
        $cantidad           = $cantidadPro;
        $precioUnitario     = $dataprod[0]->PROD_UltimoCosto;
        $subTotal           = 0;
        $tipoIgv            = 1;
        $igv                = 0;
        $igv100             = 18;
    ...
    public function registrar_prodtemporal($filter = null){
...
########################################################################################################
    ...
    public function editar_comprobantes_temproductos(){
        ...
        switch ($tipo_docu) {
            ...
            case 'B':
                ...
                break;
            case 'I':
                $datos = $this->introducir_prodtemporal_importacion($idcomprobante,$tempSession);
                echo json_encode($datos);
            break;
            case 'N':
                    $datos = $this->introducir_prodtemporal_comprobante($idcomprobante,$tempSession);
    ...
    public function introducir_prodtemporal_pedido($comprobante,$session){
        ...
        if (count($listado_detalle) > 0) {
            foreach ($listado_detalle as $indice => $valor) {
                    ...
                    $conigv = $valor->PEDIDETC_PCIGV;
                    $sinigv = $valor->PEDIDETC_PSIGV;
                ...
                $filter->LOTP_Codigo        = 0;
                $filter->MONED_Codigo       = $moneda;
                $filter->ALMAP_Codigo       = $compania;
                $filter->TEMPDE_Costo       = 0;
                $filter->TEMPDE_Stock       = "";
                $filter->TEMPDE_Cantidad    = $cantidad;
                $filter->TEMPDE_Pendiente   = 0;

                $filter->TEMPDE_Precio      = $conigv;
                
                $filter->TEMPDE_Subtotal    = $sinigv;
                $filter->TEMPDE_Descuento   = 0;
                $filter->TEMPDE_Igv         = $igv;
                

                $filter->TEMPDE_TipoIgv     = 1;
                $filter->TEMPDE_Total       = $total;
                $filter->TEMPDE_Igv100      = 18;
                $filter->TEMPDE_Descuento100= 0;
                $filter->TEMPDE_Descripcion = $nombre_producto;
                $filter->TEMPDE_Observacion = $observacion;
                $filter->TEMPDE_FlagBs      = $flagBS;
    ...
    public function introducir_prodtemporal_importacion($comprobante,$session){
        //$tempSession   = $this->input->post('tempSession');
        //$session       = $this->somevar['temp_session'];
        $compania = $this->somevar['compania'];
        $flagIgv = $this->temporaldetalle_model->obtener_flagIgv($compania);

        $datos_comprobante = $this->importacion_model->obtener_comprobante($comprobante);
        $detalle           = $this->importaciondetalle_model->get($comprobante);
        ...
        if (count($detalle) > 0) {
            foreach ($detalle as $indice => $valor) {
                ...
                $nombre_producto = $valor->IMPORDEC_Descripcion;
                $codigo_interno = $datos_producto[0]->PROD_CodigoUsuario;
                $codigo_usuario = $datos_producto[0]->PROD_CodigoUsuario;
                $nombre_unidad = is_array($datos_unidad) ? $datos_unidad[0]->UNDMED_Descripcion : 'SERV';
...
########################################################################################################
    ...
    public function cantidad_cotizaciones_articulo(){
     ...
     public function cantidad_compras_articulo(){
        $producto = $this->input->post('producto');
      //  $cliente = $this->input->post('cliente');
        $listarCompras= $this->temporaldetalle_model->listarCompras($producto);
        echo json_encode($listarCompras);
    }
