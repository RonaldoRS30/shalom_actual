...
public function buscar_comprobantes($tipo_oper = 'C', $tipo_docu = 'I',$filter = NULL, $number_items = '', $offset = '')
    {
        $compania = $this->somevar['compania'];
        ...
        if ( isset($filter->count) && $filter->count == true ){
        	$cols = "COUNT(DISTINCT cp.IMPOR_Codigo) as registros";
        	$limit = "";
        }else{
            $cols = "cp.*,
                       (CASE " . ($tipo_oper != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . "  WHEN '1'THEN e.EMPRC_RazonSocial ELSE CONCAT(pe.PERSC_Nombre , ' ', pe.PERSC_ApellidoPaterno, ' ', pe.PERSC_ApellidoMaterno) end) nombre,
        ...
        $sql = "SELECT $cols
                ...
                LEFT JOIN cji_empresa e ON e.EMPRP_Codigo=c.EMPRP_Codigo AND " . ($tipo_oper != 'C' ? "c.CLIC_TipoPersona" : "c.PROVC_TipoPersona") . " = '1'
                WHERE cp.IMPOR_TipoOperacion='" . $tipo_oper . "'
                     AND cp.IMPOR_FlagEstado > 0
                      AND cp.COMPP_Codigo =" . $compania . " " . $where . " ".$order." ". $limit;  //cp.CPC_Serie DESC, cp.CPC_Numero DESC
        $query = $this->db->query($sql);
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
...
WHERE  o.OCOMP_Codigo =  '66'*/
  ...
  //  SELECT * FROM (`cji_importacion`) WHERE `OCOMP_Codigo` = '66'
    ...
    public function getProdCosteo($imporcodigo,$impordeta){
        $sql = "SELECT * FROM cji_prodcosteo WHERE IMPOR_Codigo = $imporcodigo AND IMPORDEP_Codigo = $impordeta";
        $query = $this->db->query($sql);
    ...
    public function aprobar_importacion($imporcodigo){
        $data = array(
            'IMPOR_FlagEstado' => 1
        );

        $this->db->where("IMPOR_Codigo",$imporcodigo);
        $result = $this->db->update("cji_importacion",$data);
        if($result){
            return $result;
        }
    }
    ...
    public function insertar_imposerv($filter) {
        $this->db->insert("cji_importacionservicios", (array) $filter);
        $imposerv = $this->db->insert_id();
        return $imposerv;
    }

    public function insertardetaServ($filter) {
        $this->db->insert("cji_prodcosteo", (array) $filter);
        $imposerv = $this->db->insert_id();
        return $imposerv;
    }
...
WHERE od.OCOMP_Codigo=$codigo and od.PROD_Codigo=$prod and id.PROD_Codigo=$prod ";
    ...
    public function servicios_importacion($impor_codi){
        $sql = "SELECT * FROM cji_importacionservicios WHERE IMPOR_Codigo = $impor_codi AND IMPSER_FlagEstado = 1";
        $query = $this->db->query($sql);
        if ($query->num_rows > 0) {
            foreach ($query->result() as $fila) {
                $data[] = $fila;
    ...
    public function anularImportacion($importacion){
        $data = array(
            'IMPOR_FlagEstado' => 0
        );

        $this->db->where("IMPOR_Codigo",$importacion);
        $result = $this->db->update("cji_importacion",$data);
        if($result){
            return $result;
        }
