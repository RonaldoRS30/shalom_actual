...
class Comprobante_model extends Model {
    ...
    public function getComprobantes($filter = NULL){
        ...
        if (isset($filter->invoice) && $filter->invoice != '') {
            $where .= ' AND cp.INV_FlagEstado=' . $filter->invoice;
        }
        ...
        if ( isset($filter->count) && $filter->count == true ){
        	...
        	$cols = "cp.CPC_FechaRegistro, DATE(cp.CPC_FechaRegistro) CPC_FechaR, cp.CPC_Fecha, cp.CPP_Codigo, cp.CPC_Serie, cp.CPC_Numero, cp.CPP_Codigo_canje, cp.CPC_GuiaRemCodigo, cp.CPC_DocuRefeCodigo, cp.CPC_NombreAuxiliar, cp.CLIP_Codigo, cp.CPC_TipoDocumento, cp.CPC_total, cp.CPC_FlagEstado, cp.OCOMP_Codigo, cp.DUA_Codigo, 
                       ...
                       END as seriOC,
                       (SELECT PEDIP_Codigo FROM cji_comprobantepedido c WHERE c.CPP_Codigo = cp.CPP_Codigo LIMIT 1) as PEDIP_Codigo
                ";
  ...
  public function buscar_comprobante_venta_3($anio ,$mes ,$fech1 ,$fech2 ,$tipodocumento) {
    ...
    public function obtener_comprobante($comprobante) {
        $sql = "SELECT c.*, oc.OCOMC_PersonaAutorizada as ordenCompra,
                ...
                (SELECT p.PAIS_Descripcion FROM cji_paises p WHERE p.PAIS_Codigo = c.PAIS_Codigo) as pais_nombre,
                (SELECT p.PAIS_Iso FROM cji_paises p WHERE p.PAIS_Codigo = c.PAIS_Codigo) as pais_iso
...
//FIN ANULACION
    ...
    public function autocompletePais($keyword){
        try {
            $compania = $this->compania;
            $sql = "SELECT * FROM cji_paises WHERE PAIS_Descripcion LIKE '%$keyword%'";  
