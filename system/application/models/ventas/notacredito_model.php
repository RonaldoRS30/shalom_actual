...
class Notacredito_model extends Model
{
    ...
    public function sunatEnviado($notaCredito, $datos)
    {
        ...
        if ($valor) {
            // Obtenemos la nota de crédito para identificar el comprobante inicial
            $nota = $this->db->where('CRED_Codigo', $notaCredito)
                            ->get('cji_nota')
                            ->row();
                            
            if ($nota && $nota->CRED_ComproInicio) {
                // Manejamos las cuentas y pagos asociados al comprobante original
                $cuentas = $this->cuentas_model->getCuentaComprobante($nota->CRED_ComproInicio);
                if ($cuentas != NULL) {
                    foreach ($cuentas as $cuenta) {
                        // Obtenemos los pagos asociados a la cuenta
                        $cuentaspago_datos = $this->cuentaspago_model->obtener($cuenta->CUE_CodDocumento);
                        if (count($cuentaspago_datos) > 0) {
                            // Anulamos cada pago asociado
                            foreach ($cuentaspago_datos as $pago) {
                                $this->pago_model->anular($pago->PAGP_Codigo);
                            }
                        }
                        $tipN = $nota->DOCUP_Codigo;
                        if($tipN == 1 || $tipN == 2 || $tipN == 6){
                            // Anulamos la cuenta de pago
                            $this->cuentaspago_model->eliminar($cuenta->CUE_Codigo);
                            // Anulamos la cuenta
                            $this->cuentas_model->eliminar($cuenta->CUE_Codigo);
                        }else if($tipN == 4 || $tipN == 5 || $tipN == 7 || $tipN == 8 || $tipN == 9){

                            $filter = new stdClass();
                            $filter->CPAGC_Monto = $nota->CRED_total;
                            $this->cuentaspago_model->actualizar($filter, $cuenta->CUE_Codigo);
                            // Anulamos la cuenta

                            $filter2 = new stdClass();
                            $filter2->CUE_Monto = $nota->CRED_total;
                            $this->cuentas_model->actualizar($filter2, $cuenta->CUE_Codigo);

                        }
                    }
                }
/*                 $this->db->where('CPP_Codigo', $nota->CRED_ComproInicio)
                ->update('cji_comprobante', array('CPC_FlagEstado' => '0')); */
            }
            //#todo CPC_FlagEstado esta bien, por ahora no activarlo, tener un mejor analisis. 
    ...
    {
        // Obtenemos la nota antes de cambiar su estado para tener el comprobante original
        $nota = $this->db->where('CRED_Codigo', $comprobante)
                        ->get('cji_nota')
                        ->row();
        ...
        if ($valor && $nota && $nota->CRED_ComproInicio) {
            // Restauramos las cuentas y pagos asociados al comprobante original
            $cuentas = $this->cuentas_model->getCuentaComprobante($nota->CRED_ComproInicio);
            if ($cuentas != NULL) {
                foreach ($cuentas as $cuenta) {
                    // Restauramos la cuenta principal
                    $data_cuenta = array("CUE_FlagEstado" => '1');
                    $this->db->where("CUE_Codigo", $cuenta->CUE_Codigo);
                    $this->db->update('cji_cuentas', $data_cuenta);
    
                    // Restauramos la cuenta de pago
                    $data_cuenta_pago = array("CPAGC_FlagEstado" => '1');
                    $this->db->where("CUE_Codigo", $cuenta->CUE_Codigo);
                    $this->db->update('cji_cuentaspago', $data_cuenta_pago);
    
                    // Restauramos los pagos asociados
                    $cuentaspago_datos = $this->cuentaspago_model->obtener($cuenta->CUE_CodDocumento);
                    if (count($cuentaspago_datos) > 0) {
                        foreach ($cuentaspago_datos as $pago) {
                            $data_pago = array("PAGC_FlagEstado" => '1');
                            $this->db->where("PAGP_Codigo", $pago->PAGP_Codigo);
                            $this->db->update('cji_pago', $data_pago);
                        }
                    }
                }
            }
    ...
    #todo metodo antiguo si hay problemas descomentarlo.
/*     public function sunatEnviado($notaCredito, $datos)
    {
        $data = array(
            'CRED_ComproFin' => $datos->CPP_codigo,
