...
public function updateSolicitudCotizacion($pedido){
  ...
  function listar_pedidos_todos($filter = '',$tipo_oper = 'ALL'){
    $compania = $this->somevar['compania'];
    $hoy= DATE("Y-m-d");
    ...
    if (isset($filter->fechai) && $filter->fechai != '' )
      $where = ' and pe.PEDIC_FechaRegistro BETWEEN "' . $filter->fechai . ' 00:00:00" AND "' . $hoy . ' 23:59:59"';
    if (isset($filter->fechaf) && $filter->fechaf != '')
      $where = ' and pe.PEDIC_FechaRegistro BETWEEN "' . $filter->fechai . ' 00:00:00" AND "' . $filter->fechaf . ' 23:59:59"';
    ...
    if (isset($filter->nombre_cliente) && $filter->nombre_cliente != '')
		    $where .= " AND CONCAT_WS(' ',c.CLIC_CodigoUsuario, p.PERSC_Nombre, p.PERSC_ApellidoPaterno, e.EMPRC_RazonSocial) LIKE '%".$filter->nombre_cliente."%'";
		if(isset($filter->ruc_cliente) && $filter->ruc_cliente != '')
		    $where .= " AND CONCAT_WS(' ',p.PERSC_NumeroDocIdentidad, e.EMPRC_Ruc) LIKE '%".$filter->ruc_cliente."%'";
    if(isset($filter->estado) && $filter->estado != ''){
      $where .= "AND pe.PEDIC_FlagEstado = ".$filter->estado;
    }
    ...
    switch ($tipo_oper) {
      ...
      $sql = "SELECT DISTINCT pe.*, pr.PRESUC_Serie, pr.PRESUC_Numero, oc.OCOMC_Serie, oc.OCOMC_Numero,
                ...
                (CASE c.CLIC_TipoPersona WHEN 1 THEN e.EMPRC_RazonSocial ELSE CONCAT(p.PERSC_Nombre,' ',p.PERSC_ApellidoPaterno,' ',p.PERSC_ApellidoMaterno) end) as nombre,
                (SELECT GUIAREMP_Codigo FROM cji_guiarempedido gr WHERE gr.PEDIP_Codigo = pe.PEDIP_Codigo LIMIT 1) as GUIAREMP_Codigo,
                (SELECT CONCAT_WS('-', gr.GUIAREMC_Serie, gr.GUIAREMC_Numero) FROM cji_guiarempedido gr WHERE gr.PEDIP_Codigo = pe.PEDIP_Codigo LIMIT 1) as GUIAREMC_SerieNumero,
                (SELECT gr.GUIAPED_flagEstado FROM cji_guiarempedido gr WHERE gr.PEDIP_Codigo = pe.PEDIP_Codigo LIMIT 1) as GUIAREMC_FlagEstado,
                (SELECT CPP_Codigo FROM cji_comprobantepedido c WHERE c.PEDIP_Codigo = pe.PEDIP_Codigo LIMIT 1) as CPP_Codigo,
                (SELECT CONCAT_WS('-', c.CPC_Serie, c.CPC_Numero) FROM cji_comprobantepedido c WHERE c.PEDIP_Codigo = pe.PEDIP_Codigo LIMIT 1) as CPC_SerieNumero,
                (SELECT c.COMPPE_FlagEstado FROM cji_comprobantepedido c WHERE c.PEDIP_Codigo = pe.PEDIP_Codigo LIMIT 1) as CPC_FlagEstado
                FROM cji_pedido pe
  ...
  function listar_total_productos_pedido($pedido){
    ...
    public function obtener_pedido($pedido){
      $sql = "SELECT p.*,
                ...
                (SELECT GUIAREMP_Codigo FROM cji_guiarempedido gr WHERE gr.PEDIP_Codigo = p.PEDIP_Codigo LIMIT 1) as GUIAREMP_Codigo,
                (SELECT CONCAT_WS('-', gr.GUIAREMC_Serie, gr.GUIAREMC_Numero) FROM cji_guiarem gr WHERE gr.GUIAREMP_Codigo = gp.GUIAREMP_Codigo LIMIT 1) as GUIAREMC_SerieNumero,
                (SELECT CPP_Codigo FROM cji_comprobantepedido c WHERE c.PEDIP_Codigo = p.PEDIP_Codigo LIMIT 1) as CPP_Codigo,
                (SELECT CONCAT_WS('-', c.CPC_Serie, c.CPC_Numero) FROM cji_comprobante c WHERE c.CPP_Codigo = cp.CPP_Codigo LIMIT 1) as CPC_SerieNumero
                FROM cji_pedido p
                LEFT JOIN cji_guiarempedido gp ON p.PEDIP_Codigo = gp.PEDIP_Codigo
                LEFT JOIN cji_comprobantepedido cp ON cp.PEDIP_Codigo = p.PEDIP_Codigo
                WHERE p.PEDIP_Codigo = $pedido;
              ";
...
function obtener_pedido_pedidodetalle($pedido){
  $sql = "SELECT p.*, pd.*,
            ...
            (CASE c.CLIC_TipoPersona  WHEN '1'
                THEN e.EMPRC_Direccion
                ELSE pe.PERSC_Direccion end) as direccion
            FROM cji_pedido p
            INNER JOIN cji_pedidodetalle pd ON p.PEDIP_Codigo = pd.PEDIP_Codigo
            ...
            WHERE p.PEDIP_Codigo = $pedido AND pd.PEDIDETC_FlagEstado = '1'
          ";
  $query = $this->db->query($sql);
   #$where = array("PEDIP_Codigo"  => $pedido,"PEDIDETC_FlagEstado" => "1");
   #$query = $this->db->where($where)->get('cji_pedidodetalle');
...
function pedidos_diarios($fechaini = null, $fechafin = null){
  $hoy = date("Y-m-d");
  if($fechaini == null || $fechafin == null){
    $sql = "SELECT pd.*, p.*, pe.*, fb.FABRIC_Descripcion, SUM(pd.PEDIDETC_Importe) as TotalImporte, SUM(pd.PEDIDETC_Cantidad) as TotalCantidad
            FROM cji_pedido pe
            INNER JOIN cji_pedidodetalle pd ON pd.PEDIP_Codigo = pe.PEDIP_Codigo
            INNER JOIN cji_producto p ON p.PROD_Codigo = pd.PROD_Codigo
            LEFT JOIN cji_fabricante fb ON fb.FABRIP_Codigo = p.FABRIP_Codigo
            WHERE pe.PEDIC_FlagEstado IN (2,0) GROUP BY p.PROD_Codigo, pe.ALMAP_Codigo
          ";
  }else{
    $sql = "SELECT pd.*, p.*, pe.*, fb.FABRIC_Descripcion, SUM(pd.PEDIDETC_Importe) as TotalImporte, SUM(pd.PEDIDETC_Cantidad) as TotalCantidad
            FROM cji_pedido pe
            ...
            INNER JOIN cji_producto p ON p.PROD_Codigo = pd.PROD_Codigo
            LEFT JOIN cji_fabricante fb ON fb.FABRIP_Codigo = p.FABRIP_Codigo
            WHERE pd.PEDIDETC_FechaRegistro BETWEEN '$fechaini' AND '$fechafin' AND pe.PEDIC_FlagEstado IN (2,0) GROUP BY p.PROD_Codigo, pe.ALMAP_Codigo
          ";
  }
...
public function buscarPedidoGuiarem($pedido,$estadoAsociacion){
  $this->db->from('cji_guiarempedido gp');
  $this->db->join('cji_guiarem g', 'g.GUIAREMP_Codigo=gp.GUIAREMP_Codigo');
  
  if($estadoAsociacion!=null && trim($estadoAsociacion)!="")
      $this->db->where("gp.GUIAPED_flagEstado =",$estadoAsociacion);
  
  
  $this->db->where("gp.PEDIP_Codigo",$pedido);
  $query = $this->db->get();
  if ($query->num_rows > 0) {
      foreach ($query->result() as $fila) {
          $data[] = $fila;
...
public function ultimo_numero(){
  ...
  function insertar_pedido($filterP){
      $this->db->insert("cji_pedido", (array)$filterP);
      $pedido_cod = $this->db->insert_id();
      return $pedido_cod;
  }
    ...
    function modificar_pedido($pedido,$filter=null){
      ...
      $this->db->where($where);
      $result = $this->db->update('cji_pedido', (array) $filter);
      if($result){
        return $result;
      }
    ...
    function modificar_pedido_por_guiarem($guiarem,$filter=null){
      
      $compania = $this->somevar['compania'];
      $usuario =  $this->somevar['usuario'];
      $codigo = "SELECT PEDIP_Codigo FROM cji_guiarempedido WHERE GUIAREMP_Codigo = $guiarem";
      $query = $this->db->query($codigo);
      $pedido = $query->result();
      $filter->PEDIC_FechaModificacion= date('Y-m-d h:i:s');
      
      $where = array("PEDIP_Codigo" => $pedido[0]->PEDIP_Codigo);
      $this->db->where($where);
