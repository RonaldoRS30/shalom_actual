...
//     	$query = $this->db->where('PERSC_NumeroDocIdentidad', $ordAdj)->get('cji_persona');
    ...
    public function obtenerCodigopersxcoddirectivo($id) {
        $sql = "SELECT PERSP_Codigo FROM cji_directivo WHERE DIREP_Codigo = ?";
        $query = $this->db->query($sql, array($id)); 
        
        if ($query->num_rows() > 0) {
            return $query->row()->PERSP_Codigo; // Devuelve el valor si existe
        } 
