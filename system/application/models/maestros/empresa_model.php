...
class Empresa_model extends Model{
  ...
  ############################
	...
	public function documento_exists($numero, $empresa) {
		$sql = "
			SELECT 
				(SELECT COUNT(*) FROM cji_empresa WHERE EMPRC_Ruc = '$numero' AND EMPRC_EmpCod='$empresa') +
				(SELECT COUNT(*) FROM cji_persona WHERE PERSC_NumeroDocIdentidad = '$numero' AND PERSC_EmpCod='$empresa')
			AS total_registros
		";
	
		$query = $this->db->query($sql);
		$row = $query->row(); // Obtener el resultado
	
		// Si hay 2 o más registros, se considera que el documento ya existe
		return ($row->total_registros >= 2);
	}
  ...
  ## CONTACTOS
	public function getContactos($filter = NULL){
		...
		WHERE ec.ECONC_FlagEstado LIKE '1' ".$where." ".$order." ".$limit." ";
  ...
  public function listar_contactosEmpresa($empresa){
  	$sql = "SELECT 
		cji_emprcontacto.*,
		cji_persona.PERSP_Codigo,
		cji_persona.PERSC_Nombre,
		cji_persona.PERSC_ApellidoPaterno,
		cji_persona.PERSC_ApellidoMaterno,
		cji_area.AREAP_Codigo,
		cji_area.AREAC_Descripcion,
		cji_cargo.CARGC_Descripcion
	FROM cji_emprcontacto
	LEFT JOIN cji_persona 
		ON cji_persona.PERSP_Codigo = cji_emprcontacto.ECONC_Persona
	LEFT JOIN cji_directivo 
		ON cji_directivo.EMPRP_Codigo = cji_emprcontacto.EMPRP_Codigo 
		AND cji_directivo.PERSP_Codigo = cji_emprcontacto.ECONC_Persona
	LEFT JOIN cji_cargo 
		ON cji_cargo.CARGP_Codigo = cji_directivo.CARGP_Codigo
	LEFT JOIN cji_emprarea 
		ON cji_emprarea.EMPRP_Codigo = cji_emprcontacto.EMPRP_Codigo 
		AND cji_emprarea.DIREP_Codigo = cji_directivo.DIREP_Codigo
	LEFT JOIN cji_area 
		ON cji_area.AREAP_Codigo = cji_emprarea.AREAP_Codigo
	WHERE 
		cji_emprcontacto.ECONC_FlagEstado = '1' 
		AND cji_emprcontacto.EMPRP_Codigo = $empresa
	GROUP BY cji_emprcontacto.ECONP_Contacto
	ORDER BY cji_emprcontacto.ECONP_Contacto";
	$query = $this->db->query($sql);
