...
class Temporaldetalle_Model extends Model {
    ...
    public function listarCotizaciones( $producto = NULL, $cliente = NULL ){
        ...
        public function listarCompras( $producto = NULL){
        ...
        $sql = "SELECT c.CPC_TipoDocumento as documento, c.CPC_Serie as serie, c.CPC_Numero as numero, cd.CPDEC_Descripcion as nombreArticulo, cd.CPDEC_Pu_ConIgv as precioCigv, cd.CPDEC_Pu as precioSigv, cd.CPDEC_FechaRegistro as fecha
                FROM cji_comprobante c
                INNER JOIN cji_comprobantedetalle cd ON c.CPP_Codigo = cd.CPP_Codigo
                WHERE 
                cd.PROD_Codigo = $producto AND c.COMPP_Codigo = $compania AND c.CPC_TipoOperacion = 'C' AND
                        c.CPC_FlagEstado = 1 AND cd.CPDEC_FlagEstado = 1 
                        ORDER BY cd.CPDEC_FechaRegistro DESC
                        LIMIT 1;";
