...
class Almacenproducto_Model extends Model {
    ...
    public function colocar_stock_comprometido($almacen_id, $producto_id, $cantidad, $pcosto = '') {
        $filter = new stdClass();
        $filter->COMPP_Codigo = $this->compania;
        $filter->ALMAC_Codigo = $almacen_id;
        $filter->PROD_Codigo = $producto_id;
        ...
        if (count($stock) > 0) {
            $almacenprod_id = $stock[0]->ALMPROD_Codigo;
            $cantidad_total = $stock[0]->ALMPROD_StockComprometido + $cantidad;

            $filter->ALMPROD_StockComprometido = $cantidad_total;
            $this->db->where("ALMPROD_Codigo", $almacenprod_id);
            $this->db->update("cji_almacenproducto", (array) $filter);
        } else {
            $filter->ALMPROD_StockComprometido = $cantidad;
            $filter->ALMPROD_CostoPromedio = $pcosto;
            $this->db->insert("cji_almacenproducto", (array) $filter);
            $almacenprod_id = $this->db->insert_id();
        }
    ...
    public function modificar_stock_comprometido($almacen_id, $producto_id, $cantidad, $actualcant, $pcosto = '') {
        $filter = new stdClass();
        $filter->COMPP_Codigo = $this->compania;
        $filter->ALMAC_Codigo = $almacen_id;
        $filter->PROD_Codigo = $producto_id;
        ...
        if (count($stock) > 0) {
            $almacenprod_id = $stock[0]->ALMPROD_Codigo;
            $cantidad_total = ($stock[0]->ALMPROD_StockComprometido + $cantidad)-$actualcant;
    ...
    public function AprobarPedido($almacen_id, $producto_id, $cantidad) {
        $stock = $this->obtener($almacen_id, $producto_id);
        if (count($stock) > 0) {
            $almacenprod_id = $stock[0]->ALMPROD_Codigo;
            $cantidad_original = $stock[0]->ALMPROD_Stock;
            $comprometida_original = $stock[0]->ALMPROD_StockComprometido;
            $costo_anterior = $stock[0]->ALMPROD_CostoPromedio;
            if ($cantidad != $cantidad_original) {
                $cantidad_total = $cantidad_original - $cantidad;
            } else {
            ...
            $comprometida_total = $comprometida_original - $cantidad;
            $compania = $this->compania;
            $filter = new stdClass();
            $filter->ALMAC_Codigo = $almacen_id;
            $filter->PROD_Codigo = $producto_id;
            $filter->COMPP_Codigo = $compania;
            $filter->ALMPROD_StockComprometido = $comprometida_total;
            $this->db->where("ALMPROD_Codigo", $almacenprod_id);
            $this->db->update("cji_almacenproducto", (array) $filter);
            //Disminuyo stock a la tabla producto
            $datos_producto = $this->producto_model->obtener_producto($producto_id);
    ...
    public function AnularPedido($almacen_id, $producto_id, $cantidad){
        $stock = $this->obtener($almacen_id, $producto_id);
        $almacenprod_id = $stock[0]->ALMPROD_Codigo;
        $comprometida_original = $stock[0]->ALMPROD_StockComprometido;
