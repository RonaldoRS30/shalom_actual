...
class Lib_props
{
  ...
  public function pedido_pdf($codigo = null, $flagPdf = 1, $fechaini, $fechafin, $enviarcorreo = false)
  {
    /* Datos principales */
    if($codigo == "cons"){
      $datos_pedido = $this->ci->pedido_model->pedidos_diarios($fechaini, $fechafin);
    }else{
      $datos_pedido = $this->ci->pedido_model->obtener_pedido($codigo);
      $datos_detalle_pedido = $this->ci->pedidodetalle_model->listar1($codigo);
      $estado = 4;
    }
      $tipo_oper = "V1";
      ...
      $subtotal = $datos_pedido[0]->PEDIC_PrecioTotal;
      $descuentototal = $datos_pedido[0]->OCOMC_descuento;
      $igvtotal = $datos_pedido[0]->PEDIC_IGVTotal;
      $observacion = $datos_pedido[0]->PEDIC_Observacion;
      $fechaEntregaMin = $datos_pedido[0]->PEDIC_FechaEntregaMin;
      $fechaEntregaMax = $datos_pedido[0]->PEDIC_FechaEntregaMax;
      $direccion = $datos_pedido[0]->PEDIC_Direccion;
      $estado = $datos_pedido[0]->PEDIC_FlagEstado;

      switch($estado){
        case 0:
          $estadoSpan = "EN ESPERA";
          break;
        case 1:
          $estadoSpan = "ENTREGADO";
          break;
        case 2:
          $estadoSpan = "EN PROCESO";
          break;
        case 3:
          $estadoSpan = "ANULADO";
          break;
    ...
    if($fechaEntregaMin != "0000-00-00 00:00:00"){
      $arrfecha = explode("-", $fechaEntregaMin);
      $nFecha = explode(' ', $arrfecha[2]);
      $fechaEntMin = $nFecha[0] . " de " . ucfirst(strtolower($this->mesesEs($arrfecha[1]))) . " del " . $arrfecha[0] . "  " . $this->formatHours($nFecha[1]);
    }else{
      $fechaEntregaMin == "";
    }

    if($fechaEntregaMax != "0000-00-00 00:00:00"){
      $arrfecha = explode("-", $fechaEntregaMax);
      $nFecha = explode(' ', $arrfecha[2]);
      $fechaEntMax = $nFecha[0] . " de " . ucfirst(strtolower($this->mesesEs($arrfecha[1]))) . " del " . $arrfecha[0] . "  " . $this->formatHours($nFecha[1]);
    }else{
      $fechaEntregaMax == "";
    }

    $arrfecha = explode("-", $datos_pedido[0]->PEDIC_FechaRegistro);
    $nFecha = explode(' ', $arrfecha[2]);
    $fecha = $nFecha[0] . " de " . ucfirst(strtolower($this->mesesEs($arrfecha[1]))) . " del " . $arrfecha[0] . "  " . $this->formatHours($nFecha[1]);
 
    $fecha1ini = explode("T",$fechaini);
    $fecha2ini = explode("-",$fecha1ini[0]);

    $fecha1fin = explode("T",$fechafin);
    $fecha2fin = explode("-",$fecha1fin[0]);
    ...
    if ($tipo_oper == "V1") {
      $tipoDocumento = "ORDEN DE<br>PEDIDO";
      $tipoDocumentoF = "COTIZACION ELECTRÓNICA";
    }

    if ($tipo_oper == "V") {
      $tipoDocumento = "ORDEN DE<br>COTIZACION";
      $tipoDocumentoF = "COTIZACION ELECTRÓNICA";
    }
    ...
    if($codigo == "cons"){
      $tipoDocumento = "CONSOLIDADO DE<br>PEDIDOS";
      $tipoDocumentoF = "CONSOLIDADO DE PEDIDOS";
    }


    $medidas = "a4";
    $this->pdf = new pdfPedidos('P', 'mm', $medidas, true, 'UTF-8', false);
    $this->pdf->SetMargins(7, 50, 10);
    if($codigo != "cons"){
      $this->pdf->SetTitle($tipoDocumentoF . ' ' . $serie . '-' . $numero);
    }else{
      $this->pdf->SetTitle($tipoDocumentoF . ' ' . $fecha1ini[0] . '-' . $fecha1fin[0]);
    }
    ...
    if($codigo == "cons"){
      ...
      $header->serie  = $fecha2ini[2]."/".$fecha2ini[1];
      $header->numero = $fecha2fin[2]."/".$fecha2fin[1];
    }else{
      $header->serie  = $serie;
    ...
    if($codigo != "cons"){
      foreach ($datos_detalle_pedido as $indice => $valor) {
        $producto = $this->ci->producto_model->getProducto($valor->PROD_Codigo);
        $nombre_producto = $producto[0]->PROD_Nombre;
        ...
        $detaProductos = $detaProductos . '
              <tr bgcolor="' . $bgcolor . '">
              <td style="' . $styleBorder . ' text-align:center;">' . $producto[0]->PROD_CodigoUsuario . '</td>
              <td style="' . $styleBorder . ' text-align:left;">' . $nombre_producto . '</td>
              <td style="' . $styleBorder . ' text-align:center;">' . $valor->unidad_medida . '</td>
              <td style="' . $styleBorder . ' text-align:center;">' . $valor->PEDIDETC_Cantidad . '</td>
              <td style="' . $styleBorder . ' text-align:right;">' . number_format($valor->PEDIDETC_PCIGV, 2) . '</td>
              <td style="' . $styleBorder . ' text-align:right;">' . number_format($valor->PEDIDETC_Importe, 2) . '</td>
              </tr>';
              
      }
    }else{
      foreach ($datos_pedido as $indice => $valor) {
        $codProducto = $valor->PROD_CodigoUsuario;
        $producto = $valor->PROD_Nombre;
        $marca = $valor->MARCC_Descripcion;
        $cantidad = $valor->TotalCantidad;
        $total = number_format($valor->TotalImporte,2);

        $bgcolor = ($indice % 2 == 0) ? "#FFFFFF" : "#F1F1F1";
        $detaProductos = $detaProductos . '
              <tr bgcolor="' . $bgcolor . '">
              <td style="' . $styleBorder . ' text-align:center;">' . $codProducto. '</td>
              <td style="' . $styleBorder . ' text-align:left;">' . $producto . '</td>
              <td style="' . $styleBorder . ' text-align:center;">' . $marca . '</td>
              <td style="' . $styleBorder . ' text-align:center;">' . $cantidad . '</td>
              <td style="' . $styleBorder . ' text-align:right;">' . $total . '</td>
              </tr>';
      }
    }
    ...
    if($codigo == "cons"){
      foreach ($datos_pedido as $indice => $valor) {
        ...
        $producto = $valor->PROD_Nombre;
        $codigoP = $valor->PROD_Codigo;
        $cantidad = $valor->TotalCantidad;
        $almacenprod = $this->ci->almacenproducto_model->obtener(null, $codigoP);
        $stock = $almacenprod[0]->ALMPROD_Stock;
        ...
        $detaStock = $detaStock . '
              ...
              <td style="' . $styleBorder . ' text-align:left;">' . $producto . '</td>
              <td style="' . $styleBorder . ' text-align:center;">' . $cantidad . '</td>
              <td style="' . $styleBorder . ' text-align:center;">' . $stock. '</td>';
              if($stockRestante < 0){
                $detaStock = $detaStock . '<td style="' . $styleBorder . ' text-align:right;color:red;">' . $stockRestante . '</td>';
              }else{
                $detaStock = $detaStock . '<td style="' . $styleBorder . ' text-align:right;">' . $stockRestante . '</td>';
              }
              $detaStock = $detaStock . '</tr>';
      }
    }
    ...
    if($codigo != "cons"){
      ...
      $clienteHTML = '<table style="font-size:8pt;" cellpadding="0.1cm" border="0">'.
            ...
            <tr>
                ...
                <td style="width:auto; text-indent:0.1cm; text-align:justification">' . $fecha . '</td>
            </tr>
            <tr>
                <td style="width:3.0cm; font-style:normal; font-weight:bold;">FECHA DE ENTREGA:</td>
                <td style="width:auto; text-indent:0.1cm; text-align:justification">' . $fechaEntMin .' - '. $fechaEntMax.'</td>
            </tr>
            <tr> 
                <td style="width:3.0cm; font-style:normal; font-weight:bold;">ESTADO:</td>
                <td style="width:auto; text-indent:0.1cm; text-align:justification">' . $estadoSpan . '</td>
            </tr>
        </table>';
      ...
      $productoHTML = '
          <table cellpadding="0.05cm" style="font-size:8.5pt; width:100%; border-collapse:collapse;" 1px solid black; text-align>
              <tr bgcolor="#F1F1F1" style="font-size:8.5pt;">
                  ...
                  <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:2.5cm;">UNIDAD</th>
              ...
              <tr>
                  <td colspan="3" style="border: 1px solid black; text-align: center; height: 2cm; width: 51%;">ENTREGADO:</td>
                  <td colspan="4" style="border: 1px solid black; text-align: center; height: 2cm; width: 50%;">RECIBIDO:</td>
              </tr>
          </table>';
    } else {
          $productoHTML = '
          <table cellpadding="0.05cm" style="font-size:8.5pt; width:100%; border-collapse:collapse;" 1px solid black; text-align>
                <tr bgcolor="#F1F1F1" style="font-size:8.5pt;">
                    <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:2.25cm;">Código</th>
                    <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:10.5cm;">Descripción</th>
                    <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:2.5cm;">Cantidad</th>
                    <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:2.0cm;">Stock Actual</th>
                    <th style="' . $styleBorder . ' font-style:italic; font-weight:bold; text-align:center; width:2.0cm;">Stock Final</th>
                </tr>
                ' . $detaStock . '
          </table>
          <tr>
              <td colspan="7" style="height:5cm;"></td> <!-- Ajusta la altura según sea necesario -->
          </tr>';
    }
    ...
    $observacionesHTML = ($observacion != "" && $codigo != "cons") ? '<table border="0" style="width:19.5cm; font-size:8pt;" cellpadding="0.1cm">
          <tr bgcolor="#F1F1F1">
    ...
    if ($estado == 3) {
      $this->pdf->Image(base_url() . 'images/cabeceras/anulado.png', 30, 30, 150, 150, '', '', '', false, 300);
    }
    $infoBancosHTML = "";
        $footerHTML = '<table cellspacing="0.05cm" border="0">';

        if ($tipo_oper == "V" || $tipo_oper == "C") {
          $footerHTML .= '
              <tr>
      ...
      ';
        ...
        if($codigo != "cons"){
          ...
          $nameFile = $tipoDocumentoF . " - " . $fecha1ini[0] . "-" . $fecha1fin[0] . " " . ".pdf";
        }
  ...
  public function ocompra_pdf($codigo, $flagPdf = 1, $enviarcorreo = false)
  {
    ...
    foreach ($datos_detalle_ocompra as $indice => $valor) {
      ...
      $detaProductos = $detaProductos . '
            ...
            <td style="' . $styleBorder . ' text-align:center;">' . $valor->UNDMED_Simbolo . '</td>
            <td style="' . $styleBorder . ' text-align:center;">' . $nombre_producto . '</td>
...
';
  ...
  public function comprobante_pdf_ticket($codigo, $enviarcorreo = false)
  {
    ...
    $serieOCHTML = ($serieOC != "") ? '<tr>
              <td><b>COTIZACIÓN: </b> ' . $serieOC . ' - 25</td>
              </tr>' : '';
  ...
  public function comprobante_pdf_a4($codigo, $flagPdf = 1, $enviarcorreo = false)
  {
      ...
      //TIENE CUOTAS
        ...
        if ($datos != NULL) {
            ...
            foreach ($datos as $key => $value) {
                ...
                $tiene_cuotas .=" <b>CUOTA ".$cuota ."</b> <b>IMPORTE: </b> ".$simbolo_moneda.'&nbsp;'.number_format($importe_cuota, 2)." <b>FECHA PAGO: </b> ".$fecha_de_pago." <b></b> ";
    ...
    foreach ($detalle_comprobante as $indice => $valor) {
      ...
      $detaProductos = $detaProductos . '
                ...
                <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; text-align:right;">' .$simbolo. number_format($valor->CPDEC_Descuento100, 2) . '</td>
                <td style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; text-align:right;">' .$simbolo. number_format($valor->CPDEC_Total - ($valor->CPDEC_Total * $valor->CPDEC_Descuento100 / 100), 2) . '</td>
                </tr>';
    }
    $gravada     = $gravada - $total_anticipo;
    $gravada    -= ($gravada * $descuento100 / 100);
    ...
    $clienteHTML = '<table style="text-indent:0cm;" cellpadding="0.02cm" border="0">
        ...
        <tr>
          ...
          <td style="width:53%; text-align:left">' . $direccion . '</td>
          <td style="width:1%;"></td>
          <td style="width:14%; font-weight:bold;">GUIA REMISION:</td>
          <td style="width:15%;">' . $guiaRemision . '</td>
        </tr>
        <tr>
         <td style="width:15%; font-weight:bold;">MONEDA:</td>
         <td style="width:53%;">' . $moneda_nombre . '</td>
         <td style="width:1%;"></td>
    ...
    $productoHTML = '
        ...
        <th style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; text-align:center; width:2.0cm;">UNIDAD</th>
        <th style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; text-align:center; width:2.0cm;">CÓDIGO</th>
        <th style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; text-align:center; width:7.5cm;">DESCRIPCIÓN</th>
        <th style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; text-align:center; width:1.3cm;">V/U</th>
        <th style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; text-align:center; width:1.3cm;">P/U</th>
        <th style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; text-align:center; width:2.0cm;"> % DSCTO</th>
        <th style="border-left: #cccccc 1mm solid; border-right: #cccccc 1mm solid; border-bottom:#cccccc 1mm solid; border-top:#cccccc 1mm solid; font-style:italic; font-weight:bold; text-align:center; width:1.5cm;">TOTAL</th>
        </tr>
        ' . $detaProductos . '
        </table>';
    ...
    $gravadaHTML = ($gravada > 0) ? '<tr>
        <td style="width:15cm; font-weight:bold;">Op.Gravada</td>
        <td style="width:01.0cm; font-weight:bold;">' . $simbolo_moneda . '</td>
        <td style="width:02.0cm; font-weight:bold;">' . number_format($gravada, 2) . '</td>
        </tr>' : '';

    $igvHTML = ($igv > 0) ? '<tr>
        <td style="width:15cm; font-weight:bold;">IGV 18%</td>
        <td style="width:01.0cm; font-weight:bold;">' . $simbolo_moneda . '</td>
    ...
    $totalesHTML = '<table align="right" cellspacing="0.1cm">
        ...
        ' . $importeBolsaHTML . '
        <tr>
        <td style="width:15cm; font-weight:bold;">Importe total</td>
        <td style="width:01.0cm; font-weight:bold;">' . $simbolo_moneda . '</td>
...
<table cellspacing="0" border="0" style="margin-top: 15px; margin-bottom: 10px; width: 100%;">
    ...
    $guiaRem = ($guiaRemision != '') ? '<tr>
        <td style="width:5cm;"><b>GUIA DE REMISIÓN:</b></td>
        <td style="width:15.5cm;">' . $guiaRemision . '</td>
        </tr>' : '';

    $serieOCHTML = ($serieOC != "") ? '<tr>
        <td style="width:3cm;"><b>COTIZACIÓN:</b></td>
        <td style="width:15.5cm;">' . $serieOC . ' - 25</td>
        </tr>' : '';

    $ordenCompraClienteHTML = ($ordenCompraCliente != "") ? '<tr>
        <td style="width:3cm;"><b>ORDEN DE COMPRA:</b></td>
        <td style="width:15.5cm;">' . $ordenCompraCliente . '</td>
        </tr>' : '';
    $observacionHTML = ($observacion != "") ? '<tr>
        <td style="width:2.7cm;"><b>OBSERVACIÓN:</b></td>
        <td style="width:15.5cm;">' . $observacion . '</td>
        </tr>' : '';
    $tienecuotasHTML = ($tiene_cuotas != "") ? '<tr>
        <td style="width:2.7cm;"><b>CUOTAS DE PAGO:</b></td>
        <td style="width:15.5cm;">' . $tiene_cuotas . '</td>
        </tr>' : '';

    $sujetaretencionDetHTML = ($retencion_codigo != "" && $retencion_codigo != null) ? '<tr>
        <td style="width:7.5cm;"><b>OPERACIÓN SUJETA A RETENCIÓN DEL I.G.V. TASA:</b></td>
        <td style="width:15.5cm;"><b>'.$retencion_porc.'%&nbsp;&nbsp;&nbsp; '.$simbolo_moneda .' '. number_format($total_retencion, 2).'</b></td>
        </tr>' : '';
    ...
    $othersFormasHTML = '
        <tr>
          <td style="width:2.5cm;"><b>FORMA DE PAGO:</b></td>
          <td colspan=""style="width:15.5cm;">';
          if (count($othersFormasP) > 0) {
            $othersFormasHTML .= $formapago_desc;
        } else {
            // Si solo hay una forma de pago, mostrar la descripción con el monto
            $othersFormasHTML .= $formapago_desc . ': ' . $simbolo_moneda . ' ' . number_format($montoPayMain, 2);
        }
...
$this->pdf->writeHTML($bancos_logos, true, false, true, '');
    ...
    if ($estado == 1) {
    // Establecer transparencia (alpha) entre 0 (totalmente transparente) y 1 (opaco)
 $this->pdf->SetAlpha(0.25); // Define transparencia (0 = totalmente transparente, 1 = opaco)
$this->pdf->Image(base_url() . 'images/cabeceras/aprobado.png', 15, 50, 180, 180, '', '', '', false, 300);
$this->pdf->SetAlpha(1); // Restablece transparencia para siguientes elementos
...
$this->pdf->settingHeaderData($header, $tipoDocumento);
  ...
  public function nota_pdf_a4($codigo, $flagPdf = 1, $enviarcorreo = false)
  {
    ...
    $clienteHTML = '<table style="text-indent:0cm;" cellpadding="0.02cm" border="0">
        <tr>
        <td style="width:3.2cm"><b>' . $tp . '</b></td>
        <td colspan="2" style="width:9.8cm; text-indent:-0.1cm;">'.'</td>
        ...
        <tr>
        <td style="width:3.2cm; font-weight:bold;">MONEDA:</td>
        <td style="width:9.2cm; text-indent:-0.1cm;">' . $moneda_nombre . '</td>
        </tr>
        </table>';
    ...
    if ($cuotas_total==""){
      ...
      $c_html='
        <tr>
            <td style=" width:1.5cm;"><b>CUOTAS:</b></td>
    ...
    $adicionalHTML = '<table cellspacing="0.1cm" border="0">
        <tr>
        <td style="width:3cm;"><b>MOTIVO DE EMISIÓN:</b></td>
        <td style="width:5cm;">' . $motivoNota . ' - ' . $motivoN . '</td>
        </tr>
        <tr>
        <td style="width:4.2cm;"><b>DOCUMENTO RELACIONADO:</b></td>
        <td style="width:8.6cm;">' . $tdocumento . ' ' . $referencia . '</td>
        </tr>
        <tr>
        <td style="width:2.2cm;"><b>OBSERVACIÓN:</b></td>
...
$this->pdf->settingHeaderData($header, $tipoDocumento);
  ...
  public function guiarem_pdf($codigo, $flagPdf = 0, $enviarcorreo = false)
  {
    ...
    $clienteHTML = '
          ...
          <tr>
          <td style="width:6cm; text-indent:1cm; font-weight: bold; ">FECHA EMISION:</td>
          <td style="width:3cm;">' . mysql_to_human($fecha) . '</td>
          <td style="width:6cm; font-weight: bold;">FECHA INICIO DE TRASLADO:</td>
          <td style="text-align:left;">' . $fecha_traslado . '</td>
          </tr>
          <tr>
          <td style="width:6cm; font-weight: bold; text-indent:1cm;">MOTIVO DEL TRASLADO:</td>
          <td style="width:3cm;">' . $movimiento_descripcion . '</td>
          <td style="width:6cm; font-weight: bold; ">MODALIDAD DE TRANSPORTE:</td>
          ...
          <td style="width:3cm;">' . $peso_total . '</td>
          ...
          <table style="font-size:7pt;" border="0" cellpadding="0.05cm">
          <tr bgcolor="'.$this->color.'">
          <td style="font-weight:bold; text-align: left; color: white;" colspan>DATOS DEL PUNTO DE PARTIDA Y PUNTO DE LLEGADA</td>
          </tr>
          <tr>
          ...
          <td style="width:5cm;">MARCA: '.$marca.'<br>'.'PLACA: '.$placa.'<br>'.'REGISTRO MTC: '.$registro_mtc.'</td>
          <td style="width:3cm; font-weight: bold;">CONDUCTOR:</td>
          <td style="width:3cm;">' . $nombre_conductor ." ".$apellido_conductor.'<br>'.'LICENCIA: '.$licencia. '<br>'.' DNI: ' . $recepciona_dni . '</td>
          </tr>
    ...
    $this->pdf->writeHTML($productoHTML, true, false, true, '');
            ...
            $CuentaBanco ='<tr>
            <td style="width:6cm;"><b>CUENTA CORRIENTE EN SOLES BCP:</b></td>
            <td style="width:15.5cm;"><b>194-2133864-0-80</b></td>
            <td style="width:10%;"></td>
        </tr>
        <tr>
            <td style="width:6cm;"><b>CUENTA INTERBANCARIA BCP (CCI):</b></td>
            <td style="width:15.5cm;"><b>002-19400213386408094</b></td>
            <td style="width:10%;"></td>
        </tr>';

        $this->pdf->writeHTML($CuentaBanco, false, false, true, false);
  ...
  public function excel_impor($codigo){
    $this->ci->load->library('Excel');
    ...
    $sheet->setTitle("COSTEO POR DUA");

    $columnWidths = [
      "B" => 20, "C" => 20, "D" => 20, "E" => 20, "F" => 25,
      "G" => 25, "H" => 25, "I" => 25, "J" => 25, "K" => 25, "L" => 25, "M" => 25
  ];
    // Definir estilos
    $estiloTitulo = [
      ...
      'alignment' => ['horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER],
        'fill' => [
          'type' => PHPExcel_Style_Fill::FILL_SOLID,
          'color' => ['rgb' => 'CCFFCC'] // Color verde claro
      ],
      'borders' => [
  ...
  $sheet->setCellValue("B2", "IMPORTACIÓN GENERAL");
  $sheet->mergeCells("B2:L2");
  $sheet->getStyle("B2:L2")->applyFromArray($estiloTitulo);

    $headerStyle = [
        'font' => ['bold' => true, 'size' => 12, 'color' => ['rgb' => 'FFFFFF']],
        'alignment' => ['horizontal' => PHPExcel_Style_Alignment::HORIZONTAL_CENTER],
        'fill' => ['type' => PHPExcel_Style_Fill::FILL_SOLID, 'color' => ['rgb' => '000000']],
        'borders' => ['allborders' => ['style' => PHPExcel_Style_Border::BORDER_THIN, 'color' => ['rgb' => 'FFFFFF']]]
    ];

    $sheet->getStyle("B4:K4")->applyFromArray($headerStyle);

    $sheet->setCellValue("B4", "DUA");
    $sheet->setCellValue("C4", "NUMERO");
    $sheet->setCellValue("D4", "FECHA");
    $sheet->setCellValue("E4", "MONTO");

    $fila = 5;
    $letra = "F";
    $columnasServicios=[];

    $importacionData = $this->ci->importacion_model->obtener_comprobante($codigo);

    if (!empty($importacionData)) {
        $adValoremTota = $importacionData[0]->IMPOR_advalorem;   
        $comprobantesRela = $this->ci->comprobante_model->getDuaCodigo($importacionData[0]->DUA_Codigo);

        if (!empty($comprobantesRela)) {
            $inicioFila = $fila;

            
            foreach ($comprobantesRela as $valor) {
                $comprobante = $valor->CPP_Codigo;
                $comprobanteDeta = $this->ci->comprobantedetalle_model->detalles($comprobante);

                $tieneProductoB = false;
                foreach ($comprobanteDeta as $valor2) {
                    $producto = $valor2->PROD_Codigo;
                    $dataProducto = $this->ci->producto_model->getProducto($producto);
                    $servicio = $dataProducto[0]->PROD_FlagBienServicio;
                }
                if ($servicio == "B") {
                  if ($fila == $inicioFila) {
                    $sheet->setCellValue("B{$fila}", $importacionData[0]->DUA_Codigo);
                  }

                  $sheet->setCellValue("C{$fila}", $valor->CPC_Serie . "-" . $valor->CPC_Numero);
                  $sheet->setCellValue("D{$fila}", $valor->CPC_Fecha);
                  $sheet->setCellValue("E{$fila}", $valor->CPC_total);       
                  $dataStyle = [
                      'borders' => ['allborders' => ['style' => PHPExcel_Style_Border::BORDER_THIN]],
                      'alignment' => ['vertical' => PHPExcel_Style_Alignment::VERTICAL_CENTER]
                  ];
                  $sheet->getStyle("B{$fila}:K{$fila}")->applyFromArray($dataStyle);
                  $fila++;
                }else{
                  $nombreServicio = $dataProducto[0]->PROD_Nombre;
                  $totalServicio  = $valor->CPC_total;

                  if (!isset($columnasServicios[$nombreServicio])) {
                    $columnasServicios[$nombreServicio] = $letra; 
                    $sheet->setCellValue("{$letra}4", $nombreServicio); 
                    $sheet->setCellValue("{$letra}5", $totalServicio); 
                    $letra++;
                  }
              }      
            }
            $columnasServicios["AD VALOREM"] = $letra;
            $sheet->setCellValue("{$letra}4", "AD VALOREM"); 
            $sheet->setCellValue("{$letra}5", $adValoremTota); 
            $letra++;

            foreach ($columnasServicios as $columna) {
              $sheet->mergeCells("{$columna}{$inicioFila}:{$columna}" . ($fila - 1));
              $sheet->getStyle("{$columna}{$inicioFila}:{$columna}" . ($fila - 1))
            ...
            $coloresServicios = [
              "F4B084", "A9D08E", "FFE699", "D9E1F2", "FFD966", "C6E0B4"
          ]; 
      
          $indiceColor = 0; // Controla qué color se usará para cada servicio
      
          foreach ($columnasServicios as $nombreServicio => $columna) {
              $color = $coloresServicios[$indiceColor % count($coloresServicios)];
              $indiceColor++;
              // Aplicar color a las celdas de la columna de servicios
              for ($filaData = 5; $filaData <= $fila-1; $filaData++) {
                  $sheet->getStyle("{$columna}{$filaData}")->applyFromArray([
                      'fill' => [
                          'type' => PHPExcel_Style_Fill::FILL_SOLID,
                          'color' => ['rgb' => $color]
                      ]
                  ...
                  if ($fila - 1 > $inicioFila) {
                      $sheet->mergeCells("B{$inicioFila}:B" . ($fila - 1));
                      $sheet->getStyle("B{$inicioFila}:B" . ($fila - 1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                      $sheet->getStyle("B{$inicioFila}:B" . ($fila - 1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);
                  }
              }
                $columnaCostoTotal = $letra;
                $sheet->setCellValue("{$columnaCostoTotal}4", "COSTO TOTAL");
                $sheet->setCellValue("{$columnaCostoTotal}{$inicioFila}", $importacionData[0]->IMPOR_total);
                $letra++;

                // $columnaCostoTotalDUA = $letra;
                // $sheet->setCellValue("{$columnaCostoTotalDUA}4", "COSTO TOTAL DUA");
                // $sheet->setCellValue("{$columnaCostoTotalDUA}{$inicioFila}", $importacionData->IMPOR_totalDUA);
                // $letra++;

                // $columnaTCSunat = $letra;
                // $sheet->setCellValue("{$columnaTCSunat}4", "TC SUNAT");
                // $sheet->setCellValue("{$columnaTCSunat}{$inicioFila}", $importacionData->IMPOR_tcSunat);
                // $letra++;
                
                // $columnaTotalSoles = $letra;
                // $fechita = $valor->CPC_Fecha;
                // $dolar = $this->ci->tipocambio_model->getCambio(2, $fechita , 1);
                // $costo_totali = $importacionData[0]->IMPOR_total;
                // $total_dolares = $dolar * $costo_totali;
                // $sheet->setCellValue("{$columnaTotalSoles}4", "TOTAL SOLES");
                // $sheet->setCellValue("{$columnaTotalSoles}{$inicioFila}", $total_dolares);
                // $letra++;

            if ($fila - 1 > $inicioFila) {
                foreach ([$columnaCostoTotal] as $columna) {
                    $sheet->mergeCells("{$columna}{$inicioFila}:{$columna}" . ($fila - 1));
                    $sheet->getStyle("{$columna}{$inicioFila}:{$columna}" . ($fila - 1))
                          ->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
                    $sheet->getStyle("{$columna}{$inicioFila}:{$columna}" . ($fila - 1))
              ...
              $columnasCosto = ["COSTO TOTAL", "COSTO TOTAL DUA", "TC SUNAT", "TOTAL SOLES"];
        $colorCosto = "FFFF00"; 

        foreach ([$columnaCostoTotal] as $columna) {
          
          $sheet->getStyle("{$columna}5")->applyFromArray([
              'fill' => [
                  'type' => PHPExcel_Style_Fill::FILL_SOLID,
                  'color' => ['rgb' => $colorCosto]
              ],
              'font' => ['bold' => true]
          ]);

      
          for ($filaData = $inicioFila; $filaData < $fila; $filaData++) {
              $sheet->getStyle("{$columna}{$filaData}")->applyFromArray([
                  'fill' => [
                      'type' => PHPExcel_Style_Fill::FILL_SOLID,
                      'color' => ['rgb' => $colorCosto]
      ...
      for ($filaData = 5; $filaData <= $fila; $filaData++) {
        $valorDua = $sheet->getCell("B{$filaData}")->getValue();
    
        if (!empty($valorDua)) {
            $sheet->getStyle("B{$filaData}")->applyFromArray([
    ...
    $excel->setActiveSheetIndex($hoja)->setCellValue("B$lugar",  "NUMERO");
    $excel->setActiveSheetIndex($hoja)->setCellValue("C$lugar",  "CODIGO");
    $excel->setActiveSheetIndex($hoja)->setCellValue("D$lugar",  "DESCRIPCION");
    $excel->setActiveSheetIndex($hoja)->setCellValue("E$lugar",  "FECHA");
    $excel->setActiveSheetIndex($hoja)->setCellValue("F$lugar",  "MES");
    $excel->setActiveSheetIndex($hoja)->setCellValue("G$lugar",  "T.C");
    $excel->setActiveSheetIndex($hoja)->setCellValue("H$lugar",  "MONEDA");
    $excel->setActiveSheetIndex($hoja)->setCellValue("I$lugar",  "CANTIDAD");
    $excel->setActiveSheetIndex($hoja)->setCellValue("J$lugar",  "PRECIO UNI.");
    $excel->setActiveSheetIndex($hoja)->setCellValue("K$lugar",  "VALOR");
    $sheet->getStyle("B$lugar:K$lugar")->applyFromArray($estiloColumnasTitulo);
    ...
    $importacion = $this->ci->importacion_model->obtener_comprobante($codigo);
    $serieNumImpo = $importacion[0]->IMPOR_Serie."-".$importacion[0]->IMPOR_Numero;
    $comprobantesRela = $this->ci->comprobante_model->getDuaCodigo($importacion[0]->DUA_Codigo);
    $lugar++;
    $lugarini = 3;
    $totalDUA = 0;
    $previousLetter = "K";
    ...
    foreach($comprobantesRela as $i => $valor){
      $serieNum     = $valor->CPC_Serie."-".$valor->CPC_Numero;
      $fechaCom     = $valor->CPC_Fecha;
      $total        = $valor->CPC_total;
      $datos_moneda = $this->ci->moneda_model->obtener($valor->MONED_Codigo);
      $monedaDesc   = $datos_moneda[0]->MONED_Descripcion;
      $fechaExplode = explode("-",$fechaCom);
      $mes          = $fechaExplode[1];

      $comprobantesDeta = $this->ci->comprobantedetalle_model->detalles($valor->CPP_Codigo);
      $tipocambio = $this->ci->tipocambio_model->getCambio(2, $valor->CPC_Fecha,$valor->MONED_Codigo);
      
      foreach($comprobantesDeta as $j => $valorD){
        $cantidad     = $valorD->CPDEC_Cantidad;
        $preciou      = $valorD->CPDEC_Pu;
        $valorU       = $valorD->CPDEC_Pu_ConIgv;
        $totalU       = $valorD->CPDEC_Total;

        $ProductoData = $this->ci->producto_model->getProducto($valorD->PROD_Codigo);
        $nombreProd   = $ProductoData[0]->PROD_Nombre;
        $codigoProd   = $ProductoData[0]->PROD_CodigoUsuario;
        $flagBS       = $ProductoData[0]->PROD_FlagBienServicio;
        if($flagBS == "B"){
          $excel->setActiveSheetIndex($hoja)->setCellValue("C$lugar", $codigoProd);
          $excel->setActiveSheetIndex($hoja)->setCellValue("D$lugar", $nombreProd);
          $excel->setActiveSheetIndex($hoja)->setCellValue("E$lugar", $fechaCom);
          $excel->setActiveSheetIndex($hoja)->setCellValue("F$lugar", $mes);
          $excel->setActiveSheetIndex($hoja)->setCellValue("G$lugar", $tipocambio);
          $excel->setActiveSheetIndex($hoja)->setCellValue("H$lugar", $monedaDesc);
          $excel->setActiveSheetIndex($hoja)->setCellValue("I$lugar", $cantidad);
          $excel->setActiveSheetIndex($hoja)->setCellValue("J$lugar", $valorU);
          $excel->setActiveSheetIndex($hoja)->setCellValue("K$lugar", $totalU);
          $sheet->getStyle("B$lugar:C$lugar")->applyFromArray($estiloCenter);
          $sheet->getStyle("E$lugar:I$lugar")->applyFromArray($estiloCenter);
          $sheet->getStyle("J$lugar:$letter$lugar")->applyFromArray($estiloRight);
          $lugar++;
          $totalDUA+=$total;
        }else{
          $excel->setActiveSheetIndex($hoja)->setCellValue("$letter"."2", $nombreProd);
          $sheet->getColumnDimension("$letter")->setWidth("20");
          $sheet->getStyle("$letter"."2")->applyFromArray($estiloColumnasServicio);
          $previousLetter++;
          $letter++;
          $servicios++;
        }
      ...
      if($flagBS != "S"){
        $excel->setActiveSheetIndex($hoja)->setCellValue("J$lugar", "TOTAL");
        $excel->setActiveSheetIndex($hoja)->setCellValue("K$lugar", $total);
        $sheet->getStyle("J$lugar:K$lugar")->applyFromArray($estiloColumnasTitulo);
        
        $MiFila[$i] = $lugar;

        $excel->setActiveSheetIndex($hoja)->mergeCells("B$lugarini:B$lugar")->setCellValue("B$lugarini", $serieNum);
        $lugar++;
        $lugarini = $lugar;
      }
    }
    $lugar = 3;
    $letra = "L";
    $importacionDeta = $this->ci->importaciondetalle_model->get($codigo);
    foreach($importacionDeta as $i => $valor3){
      $getServicios = $this->ci->importacion_model->getProdCosteo($codigo,$valor3->IMPORDEP_Codigo);

      foreach($MiFila as $REC){
        if($lugar == $REC){
          $lugar++;
        }
      }

      foreach($getServicios as $k => $valor4){
        $excel->setActiveSheetIndex($hoja)->setCellValue("$letra$lugar", round($valor4->PROCOST_Costeo, 2));
        $letra++;
      }
      $letra = "L";
      $lugar++;
    ...
    $count = $lugar+1;
    for($servicios; $servicios >=0; $servicios--){
      $excel->setActiveSheetIndex($hoja)->setCellValue("$letra$count",  "=SUM($letra$lugar:$letra"."3)");
      $letra++;
    }

    $count = $lugar;
    $excel->setActiveSheetIndex($hoja)->setCellValue("$letter"."2",  "CIF");
    $sheet->getStyle("$letter"."2")->applyFromArray($estiloColumnasServicio);
    $sheet->getColumnDimension("$letter")->setWidth("20");
    for($count; $count >=3; $count--){
      ...
      $excel->setActiveSheetIndex($hoja)->setCellValue("$letter$count",  "=SUM(K{$count}:$previousLetter$count)");
    }
    ...
    $excel->setActiveSheetIndex($hoja)->setCellValue("$letter"."2",  "COSTO UNITARIO");
    $sheet->getStyle("$letter"."2")->applyFromArray($estiloColumnasServicio);
    $sheet->getColumnDimension("$letter")->setWidth("20");
    for($count; $count >=3; $count--){
      foreach($MiFila as $REC){
      ...
      $excel->setActiveSheetIndex($hoja)->setCellValue("$letter$count",  "=$previousLetter$count/I$count");
    }
    $letter++;

    // Generar archivo Excel
    $filename = "IMPORTACIÓN ".$serieNumImpo.".xls";
