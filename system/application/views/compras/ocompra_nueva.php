...
function registrar_prodtemporal(codProd, tempSession, cantidad){
	url = base_url+"index.php/maestros/temporaldetalle/agregarProductoPedido"+"/"+codProd+"/"+tempSession+"/"+cantidad;
	$.ajax({
			url:url,
			data:"",
			type:'POST',
...
<?php
$pedido = false;
	foreach($CodProductos as $i => $val){
		if($val != "-" || $val != ""){
			$pedido = true
			?>
			<script>registrar_prodtemporal(<?=$val?>,<?=$tempSession?>,<?=$CantProductos[$i]?>) </script>
			
		<?php }
	}?> 
	
<script>mostrar_temporal_producto(<?=$pedido?>)</script>
