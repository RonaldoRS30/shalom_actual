                        ...
                        <div class="col-sm-11 col-md-11 col-lg-11 VentasArticulo" id="VentasArticulo" name="VentasArticulo">
                            	<span href="javascript:;" class="btn-close">X</span>
					     		<div class="detallesVentasAnteriores">
						     		...
						     		<table id="detallesCotizaciones" name="detallesCotizaciones"></table>  <br>
									<span style="font-size: 10pt; font-weight: bold; padding-bottom: 1em;">PRECIO DEL ARTICULO EN COMPRAS</span> <br>
								    <table id="detallesComprasAnteriores" name="detallesComprasAnteriores"></table>
						      </div>
					  </div>
                    </div>
...
<script type="text/javascript">
	...
	$("#linkVerVentasArticulo").click(function(){
		...
		if (producto > 0 && cliente > 0){
			...
			$.ajax({
            	...
            	url = "<?php echo base_url(); ?>index.php/maestros/temporaldetalle/cantidad_compras_articulo";
			$.ajax({
				url:url,
				type:"POST",
				...
				success:function(data){
					if (data != null){
						fila = '<tr class="detaArticulos">';
							fila += '<th width="5%">N°</th>';
							fila += '<th width="20%">ARTICULO</th>';
							fila += '<th width="10%">DOCUMENTO</th>';
							fila += '<th width="10%">SERIE/NÚMERO</th>';
							fila += '<th width="10%">PU S/IGV</th>';
							fila += '<th width="10%">PU C/IGV</th>';
							fila += '<th width="10%">FECHA REGISTRADA</th>';
						...
						fila = '<tr class="detaArticulos">';
						fila += '<td colspan="6">SIN COMPRAS REGISTRADAS.</td>';
						fila += '</tr>';
					}
					$("#detallesComprasAnteriores").append(fila);
				}
			});
	...
	function agregar_producto_temporal(){
	    ...
	    */
		...
		// if(flagBS != "S" && tipo_docu != "GRV" && tipo_docu != "GRC" && tipo_docu != "P" && tipo_docu != "I"){
		// 	fila += '<td width="4%"><div style="text-align:center;"><a href="#" id="addLotes" name="addLotes" style="color:#ffffff;" data-toggle="modal" data-target=".bd-lote" class="btn btn-success" onclick="mostrarModalLote(' + n + ','+producto+'); ">Lotes</a>';
		// }
		if(tipo_docu == "I"){
		...
		fila +='<input type="hidden" name="prod_peso[' + n + ']" id="prod_peso[' + n + ']" value="' + peso_total + '"/>';
	...
	function agregar_productotemp_modificado(fila){
	    ...
	    producto           = $("#tempde_codproducto").val();
	    nombre_producto    = $("#tempde_producto").val();
		peso_total 		   = document.getElementById("prod_peso["+fila+"]").value;
	    ...
	    if (anticipo == 1) {
		  ...
		  modificar_prodtemporal();
			...
			document.getElementById(pe).value 			= peso_total;
	...
	function mostrar_temporal_producto(pedido){
		var tipo_docu    = "<?=$tipo_docu;?>";
		if(pedido == true){
			Swal.fire({
              icon: "success",
              title: "Orden de compra creado",
              html: "<b class='color-red'></b>",
              showConfirmButton: true,
              timer: 4000
            });
		...
		$.ajax({
			...
			success:function(data){
				if (data.message=='1') {
					if(tipo_docu == "I"){
						array_servicios_imp(data.datos);
					}
	...
	function array_servicios_imp(datos){
		var deferred = $.Deferred();
		var tipo_docu    = "<?=$tipo_docu;?>";
		codigo = datos[0].TEMPDE_CodComprobante;

		var url = "<?=base_url();?>index.php/ventas/importacion/getServicios";
		$.ajax({
			url:url,
			type:"POST",
			data:{codigo:codigo},
			...
			success:function(data){	
				...
				data.forEach(i => {
					filaSE =  '<tr id="' + ns + '">';
                    filaSE += "<td></td>";
                    filaSE += "<td>"+i.IMPSER_Descripcion+"</td>";
                    filaSE += "<td>"+i.IMPSER_Cantidad+"</td>";
                    filaSE += "<td>"+i.IMPSER_Pu+"</td>";
                    filaSE += "<td>"+i.IMPSER_PuIGV+"</td>";
                    filaSE += "<td>"+i.IMPSER_Total+"</td>";
                    filaSE += "</tr>";

                    $("#servicios_asoc").append(filaSE);

                    filaSE = "<td style='display:none;'>";
                    filaSE += "<input type='hidden' value='"+i.IMPOR_Codigo+"' name='cpp_cod["+ns+"]' id='cpp_cod["+ns+"]' >";
                    filaSE += "<input type='hidden' value='"+i.PROD_Codigo+"' name='servcodigo["+ns+"]' id='servcodigo["+ns+"]' >";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_Descripcion+"' name='servnom["+ns+"]' id='servnom["+ns+"]'>";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_Cantidad+"' name='servcant["+ns+"]' id='servcant["+ns+"]'>";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_Pu+"' name='servpu["+ns+"]' id='servpu["+ns+"]'>";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_PuIGV+"' name='servpuigv["+ns+"]' id='servpuigv["+ns+"]'>";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_Total+"' name='servtotal["+ns+"]' id='servtotal["+ns+"]'>";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_Subtotal+"' name='servsubtotal["+ns+"]' id='servsubtotal["+ns+"]'>";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_Igv100+"' name='servigv100["+ns+"]' id='servigv100["+ns+"]'>";
                    filaSE += "<input type='hidden' value='"+i.IMPSER_Igv+"' name='servigv["+ns+"]' id='servigv["+ns+"]'>";
                    filaSE += "</td>";
	...
	function editar_comprobantes_temproductos(comprobante,tipo_docu){
		...
		$.ajax({
			...
			success:function(data){
				if (data.message == '1') {
					if(tipo_docu == "I"){
						$.when(array_servicios_imp(data.datos)).then(function(){
	...
	function calcular_totales_tempdetalle(){
		...
		imp			= document.getElementById('servicios_asoc');
	    ...
	    for( i=0; i<n ; i++){
			...
			if(imp == null){
				peso_total      = document.getElementById(pe).value;
				peso_totalgr += peso_total * cantidad;
			}else{
				peso_totalgr = 0;
			}
		...
		if(imp != null){
			impL = document.getElementById('servicios_asoc').rows.length;
			for(i = 0; i<impL; i++){
				sc 	= "servcant["+i+"]";
	        	st 	= "servsubtotal["+i+"]";
				precioS 			= parseFloat(document.getElementById(st).value);
		        gravada_total += precioS;
			}
		}
		...
		$("#peso_total").val(peso_totalgr); //val(peso_total)
