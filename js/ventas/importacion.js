var formatNumber = {
   ...
   function importacion_excel(importacion){
       var url = base_url + "index.php/ventas/importacion/excel_importacion/"+importacion;
       location.href = url;
   }
   ...
   jQuery(document).ready(function () {
      ...
      /* $("#help").change(function(){
       ...
       $("#nuevaComprobante").click(function () {
           url = base_url + "index.php/ventas/importacion/importacion_nueva" + "/" + tipo_oper + "/" + tipo_docu;
           location.href = url;
       });
         ...
         $('img#loading').css('visibility', 'visible');
           ...
           if (tipo_oper == 'C') {
               if ($("#numero").val() == "") {
                   ...
                   Swal.fire({
                       icon: "error",
                       title: "Ingrese el numero del documento.",
                       showConfirmButton: true
                   });
   ...
   if (cantidadGuiaRemision==0) {
       if (codigo == '')
           url = base_url + "index.php/ventas/importacion/importacion_insertar";
       else
           url = base_url + "index.php/ventas/importacion/importacion_modificar";
   ...
   function cerrar_modal(){
       $('.modal-comp').modal('toggle');
       $('.modal-backdrop').hide();
   }
   function getDuaCod(){
       dua = $("#dua_cod").val()
       url = base_url + "index.php/ventas/importacion/get_dua/" + dua
       if(dua == ""){
           Swal.fire({
               icon: "info",
           ...
           success: function(result){
               if(result == null){
                   Swal.fire({
                       icon: "info",
                       title: "No se encontró ningún comprobante con ese DUA",
                       showConfirmButton: true
                   });
                   return false;
               }else{
                   $("#comprobantes_dua tr").remove();
                   n= 0;
                   result.forEach(i => {
                       filaCI = "<tr>";
                       filaCI += "<td>"+i.CPC_Fecha+"</td>";
                       filaCI += "<td id=seriewu["+n+"]>"+i.CPC_Serie+"</td>";
                       filaCI += "<td>"+i.CPC_Numero+"</td>";
                       filaCI += "<td>"+i.documento+"</td>";
                       filaCI += "<td>"+i.nombre+"</td>";
                       filaCI += "<td><a href='javascript:;' onclick='seleccionarImportacionPro($codigo)'><img src='"+base_url+"images/icono-documentos.png' width='16' height='16' border='0' title='SeleccionarProductos' data-toggle='modal' data-target='.modal-productos' data-backdrop='static'></a></td>";
                       filaCI += "<td><a href='"+base_url+"index.php/ventas/comprobante/comprobante_ver_pdf/"+i.CPP_Codigo+"/a4' data-fancybox data-type='iframe' target='_blank'><img src='"+base_url+"images/pdf.png' width='16' height='16' border='0' title='ver A4'></a></td>";
                       filaCI += "<td><a href='javascript:;' onclick='seleccionarImportacion("+i.CPP_Codigo+")'><img src='"+base_url+"images/ir.png' width='16' height='16' border='0' title='Seleccionar'></a></td>";
                       filaCI += "</tr>";
   
                       $("#comprobantes_dua").append(filaCI);
                   });
                   $('.modal-comp').modal('toggle');
                   $('.modal-backdrop').show();
               }
           ...
           error: function(result){
               Swal.fire({
                   icon: "error",
                   title: "Ocurrió un error, contacatar con SOPORTE",
                   showConfirmButton: true
               });
               return false;
           }
   ...
   function seleccionarImportacion(codigo){
       var tempSession = $("#tempSession").val();
       url = base_url + "index.php/ventas/comprobante/getDetalle/" + codigo
       $.ajax({
           url: url,
           type:'POST',
           dataType: "json",
           success: function(result){
               ...
               $("#servicios_asoc tr").each(function() {
                   servCodigo[ns] = document.getElementById("servcodigo["+ns+"]").value;
                   ns++;
               });
   
               result.forEach(i => {
                   exist = false;
                   isEncuentra = verificarProductoTempDetalle(i.PROD_Codigo, i.ALMAP_Codigo);
                   if (n % 2 == 0) {
                       clase = "itemParTabla";
                   } else {
                       clase = "itemImparTabla";
                   ...
                   if(!isEncuentra){
                       if(i.PROD_FlagBienServicio == "B"){
                           fila = '<tr id="' + n + '" class="' + clase + '" >';
                           fila += '<td width="5%"><div align="center">' + (n+1) + '</div></td>';
   
                           fila += "<td width='35%'><div align='left'><input type='hidden' name='proddescri[" + n + "]' id='proddescri[" + n + "]' value='" + i.PROD_Nombre + "'/><span id='proddescri_span["+n+"]'>"+i.PROD_Nombre+"</span>";
                           fila += '</div></td>';
   
                           fila += '<td width="10%"><div style="text-align:center;">';
                           fila += '<input type="hidden" name="pendiente['+n+']" id="pendiente['+n+']" value="'+i.CPDEC_Cantidad+'">';
                           fila += '<input type="hidden" name="prodcantidad[' + n + ']" id="prodcantidad[' + n + ']" value="' + i.CPDEC_Cantidad + '"><span id="prodcantidad_span['+n+']"> '+parseFloat(i.CPDEC_Cantidad).toFixed(2)+'</span>';
                           fila += '</div></td>';
                           
                           fila += '<td width="10%"><div style="text-align:center;"><span id="prod_precio_span_sigv['+n+']">'+parseFloat(i.CPDEC_Pu).toFixed(2)+'</span></div></td>';
   
                           fila += '<td width="10%"><div style="text-align:center;"><span id="prod_precio_span['+n+']">'+parseFloat(i.CPDEC_Pu_ConIgv).toFixed(2)+'</span></div></td>';
   
                           fila += '<td width="10%"><div style="text-align:center;"><input type="hidden" size="5" style="text-align: right;" name="prodimporte[' + n + ']" id="prodimporte[' + n + ']" value="'+i.CPDEC_Total+'" ><span id="prodimporte_span['+n+']">'+parseFloat(i.CPDEC_Total).toFixed(2)+'</span></div></td>';
   
                           fila += '<td width="5%"><div style="text-align:center;"><a href="#" "></a>';
                           ...
                           fila += '<td width="5%"><div style="text-align:center;"><a href="#" onclick="info_calc(' + n + ');"><img src="'+base_url+'images/ingresar.png?=<?=IMG;?>" width="20" height="20"  border="0" tittle="Ver detalles"></a>';
                           fila += '<td width="6%" style="display:none;"><div align="center"><input style="text-align: right;" type="text" size="5" maxlength="10" class="cajaGeneral" value="' + i.CPDEC_Pu + '" name="prodpu[' + n + ']" id="prodpu[' + n + ']" value="0"></div></td>';
                           fila += '<td width="6%" style="display:none;"><div align="center"><input type="text" size="5" style="text-align: right;" maxlength="10" class="cajaGeneral cajaSoloLectura" name="prodprecio[' + n + ']" id="prodprecio[' + n + ']" value="'+i.CPDEC_Subtotal+'" readonly="readonly">';
                           
                           fila += '<td width="6%" style="display:none;" ><div align="center"><input type="text" style="text-align: right;" size="5" class="cajaGeneral cajaSoloLectura" name="prodigv[' + n + ']" id="prodigv[' + n + ']" value="'+i.CPDEC_Igv+'" readonly="readonly"></div></td>';
                           fila += '<td width="6%" style="display:none;" ><div align="center">';
                           fila += '<input type="hidden" value="n" name="detaccion[' + n + ']" id="detaccion[' + n + ']">';
                           fila += '<input type="hidden" name="prodigv100[' + n + ']" id="prodigv100[' + n + ']" value="' + i.CPDEC_Igv100 + '">';
                           fila += '<input type="hidden" value="" name="detacodi[' + n + ']" id="detacodi[' + n + ']">';
                           fila += '<input type="hidden" name="proddescuento100[' + n + ']" id="proddescuento100[' + n + ']" value="' + i.CPDEC_Descuento100 + '">';
   
                           fila += '<input type="hidden" name="tafectacion[' + n + ']" id="tafectacion[' + n + ']" value="' + i.AFECT_Codigo + '"/>';
                           fila += '<input type="hidden" name="idLote[' + n + ']" id="idLote[' + n + ']" value=""/>';
                           fila += '<input type="hidden" name="icbper[' + n + ']" id="icbper[' + n + ']" value="' + 0 + '"/>';
                           fila += '<input type="hidden" name="proddescuento[' + n + ']" id="proddescuento[' + n + ']"  />';
                           fila += '<input type="hidden" name="flagBS[' + n + ']" id="flagBS[' + n + ']" value="' + i.PROD_FlagBienServicio + '"/>';
                           fila += '<input type="hidden" name="prodobservacion[' + n + ']" id="prodobservacion[' + n + ']" value="' + i.CPDEC_Observacion + '"/>';
                           fila += '<input type="hidden" name="prodcodigo[' + n + ']" id="prodcodigo[' + n + ']" value="' + i.PROD_Codigo + '"/>';
                           fila += '<input type="hidden" name="produnidad[' + n + ']" id="produnidad[' + n + ']" value="' + i.UNDMED_Codigo + '"/>';
                           fila += '<input type="hidden" name="flagGenIndDet[' + n + ']" id="flagGenIndDet[' + n + ']" value="' + i.CPDEC_GenInd + '"/>';
                           fila += '<input type="hidden" name="prodstock[' + n + ']" id="prodstock[' + n + ']" value="' + 0 + '"/>';
                           fila += '<input type="hidden" name="prodcosto[' + n + ']" id="prodcosto[' + n + ']" value="' + i.CPDEC_Costo + '"/>';
                           fila += '<input type="hidden" name="almacenProducto[' + n + ']" id="almacenProducto[' + n + ']" value="' + i.ALMAP_Codigo + '"/>';
                           fila += '<input type="hidden" name="prodpu_conigv[' + n + ']" id="prodpu_conigv[' + n + ']" value="'+i.CPDEC_Pu_ConIgv+'" /> '
                           ...
                           //ANTICIPOS
                           fila += '<input type="hidden" name="anticipo[' + n + ']" id="anticipo[' + n + ']" value="0"/>';
                           ...
                           data = {
                               tempde_codproducto: i.PROD_Codigo,
                               tempde_almacenproducto: i.ALMAP_Codigo,
                               tempde_unidadmedida: i.UNDMED_Codigo,
                               tempde_producto: i.PROD_Nombre, 
                               tempSession: tempSession,
                               tempde_productocosto: i.CPDEC_Costo,
                               tempde_prodStock: 0,
                               tempde_cantidad: i.CPDEC_Cantidad,
                               tempde_precioUnitario: i.CPDEC_Pu_ConIgv,
                               tempde_subTotal: i.CPDEC_Subtotal,
                               tempde_tipoIgv: i.AFECT_Codigo,
                               tempde_igvLinea: i.CPDEC_Igv,
                               tempde_igv100: i.CPDEC_Igv100,
                               tempde_moneda: i.MONED_Codigo,
                               tempde_descuento: i.CPDEC_Descuento,
                               tempde_descuento100: i.CPDEC_Descuento100,
                               tempde_total: i.CPDEC_Total,
                               tempde_detalleItem: i.CPDEC_Observacion,
                               tempde_flagBs: "B"
                           }
                           $.ajax({
                               url:url,
                           ...
                           if(exist != true){
                               filaSE =  '<tr id="' + ns + '">';
                               filaSE += "<td></td>";
                               filaSE += "<td>"+i.PROD_Nombre+"</td>";
                               filaSE += "<td>"+i.CPDEC_Cantidad+"</td>";
                               filaSE += "<td>"+i.CPDEC_Pu+"</td>";
                               filaSE += "<td>"+i.CPDEC_Pu_ConIgv+"</td>";
                               filaSE += "<td>"+i.CPDEC_Total+"</td>";
                               filaSE += "</tr>";
   
                               $("#servicios_asoc").append(filaSE);
   
                               filaSE = "<td style='display:none;'>";
                               filaSE += "<input type='hidden' value='"+i.CPP_Codigo+"' name='cpp_cod["+ns+"]' id='cpp_cod["+ns+"]' >";
                               filaSE += "<input type='hidden' value='"+i.PROD_Codigo+"' name='servcodigo["+ns+"]' id='servcodigo["+ns+"]' >";
                               filaSE += "<input type='hidden' value='"+i.PROD_Nombre+"' name='servnom["+ns+"]' id='servnom["+ns+"]'>";
                               filaSE += "<input type='hidden' value='"+i.CPDEC_Cantidad+"' name='servcant["+ns+"]' id='servcant["+ns+"]'>";
                               filaSE += "<input type='hidden' value='"+i.CPDEC_Pu+"' name='servpu["+ns+"]' id='servpu["+ns+"]'>";
                               filaSE += "<input type='hidden' value='"+i.CPDEC_Pu_ConIgv+"' name='servpuigv["+ns+"]' id='servpuigv["+ns+"]'>";
                               filaSE += "<input type='hidden' value='"+i.CPDEC_Total+"' name='servtotal["+ns+"]' id='servtotal["+ns+"]'>";
                               filaSE += "<input type='hidden' value='"+i.CPDEC_Subtotal+"' name='servsubtotal["+ns+"]' id='servsubtotal["+ns+"]'>";
                               filaSE += "<input type='hidden' value='"+i.CPDEC_Igv100+"' name='servigv100["+ns+"]' id='servigv100["+ns+"]'>";
                               filaSE += "<input type='hidden' value='"+i.CPDEC_Igv+"' name='servigv["+ns+"]' id='servigv["+ns+"]'>";
                               filaSE += "</td>";
   
                               $("#tempde_tblbody").append(filaSE);
   
                               Swal.fire({
                                   icon: "success",
                                   title: "Se cargó el servicio correctamente, revísalo en la seccion de SERVICIOS",
                                   showConfirmButton: true,
                                   timer: 4000
                               });
                           }else{
                               Swal.fire({
                                   icon: "info",
                                   title: "El servicio ya ha sido ingresado en esta importación",
                                   showConfirmButton: true,
                                   timer: 4000
                               });
                               return false;
               ...
               if(o > 0){
                   Swal.fire({
                       icon: "info",
                       title: "Se omitieron "+o+" productos que ya se encontraban ingresados en la importación",
                       showConfirmButton: true
                   });
                   return false;
               }
   ...
   function costeo_serv_pro(){
       totalDUA = 0;
       totalImporte = 0;
       fProdT = document.getElementById('tempde_tbl').rows.length;
       fServ = document.getElementById('servicios_asoc').rows.length;
       for(var i = 0; i<fProdT; i++){
           afect = document.getElementById("modalAccion["+i+"]").value;
           if(afect != "e"){
               totalDUA += Number(document.getElementById("prodimporte["+i+"]").value);
           }
       }
       ...
       for(var i = 0; i<fProdT; i++){
           ...
           afect = document.getElementById("modalAccion["+i+"]").value;
           if(afect != "e"){
               produ = Number(document.getElementById("prodimporte["+i+"]").value);
               
               codi  = document.getElementById("prodcodigo["+i+"]").value;
               for(var j = 0; j<fServ; j++){
                   serv    = Number(document.getElementById("servtotal["+j+"]").value);
                   laformula += (produ/totalDUA) * serv;
                   getActual = (produ/totalDUA) * serv;
                   
   
                   ifExist = document.getElementById('prodservCodi[' + i + '][' + j + ']');
                   if (ifExist == null){
                       servcodigo = document.getElementById("servcodigo["+j+"]").value;
   
                       filaServPro = '<td width="6%" style="display:none;" ><div align="center">';
                       filaServPro += '<input type="hidden" name="prodservCodi[' + i + '][' + j + ']" id="prodservCodi[' + i + '][' + j + ']" value="' + codi + '"/>';
                       filaServPro += '<input type="hidden" name="servCodi[' + i + '][' + j + ']" id="servCodi[' + i + '][' + j + ']" value="' + servcodigo + '"/>';
                       console.log(getActual); 
                       filaServPro += '<input type="hidden" name="prodservTotal[' + i + '][' + j + ']" id="prodservTotal[' + i + '][' + j + ']" value="' + getActual + '"/>';
                       filaServPro += '</div></td>';
                       
                       $("#"+i).append(filaServPro);
                   }else{
                       document.getElementById('prodservTotal[' + i + '][' + j + ']').value = getActual;
                   }
               }
               newProdT = laformula + produ;
               totalImporte += newProdT;
               document.getElementById("prodimporte_span["+i+"]").innerHTML = newProdT.toFixed(2);
           }
       }
   }
   
   function calcularAdValorem(){
       adValorem = Number($("#adValorem").val());
       cif       = Number($("#importetotal").val());
       if(cif != ""){
           getImp = (cif * (adValorem/100));
           $("#advaloremspan").val(getImp.toFixed(2));
   
           TotalImp = cif + getImp;
           $("#totalimport").val(TotalImp.toFixed(2));
       }
   }
   
   function info_calc(fila){
       $("#servicio_producto tr").remove();
       prodSpan = document.getElementById("proddescri["+fila+"]").value;
       ...
       fProdT = document.getElementById('tempde_tbl').rows.length;
       fServ = document.getElementById('servicios_asoc').rows.length;
       produ = Number(document.getElementById("prodimporte["+fila+"]").value);
       for(var i = 0; i<fProdT; i++){
           afect = document.getElementById("modalAccion["+i+"]").value;
           if(afect != "e"){
               totalDUA += Number(document.getElementById("prodimporte["+i+"]").value);
       ...
       for(var j = 0; j<fServ; j++){
           serv = Number(document.getElementById("servtotal["+j+"]").value);
           servNom = document.getElementById("servnom["+j+"]").value;
           laformula = (produ/totalDUA) * serv;
           newProdT += laformula;
   
           filaSP =  '<tr id="' + j + '">';
           filaSP += "<td></td>";
           filaSP += "<td>"+servNom+"</td>";
           filaSP += "<td>"+serv+"</td>";
           filaSP += "<td>"+laformula.toFixed(2)+"</td>";
           // filaSP += "<td>"+i.CPDEC_Pu_ConIgv+"</td>";
           // filaSP += "<td><input type='hidden' value='"+i.CPDEC_Total+"' id='servtotal["+ns+"]'>"+i.CPDEC_Total+"</td>";
           filaSP += "</tr>";
           $("#servicio_producto").append(filaSP);
   
   
       }
       newProdT += produ;
   
       $("#totalservpro").html(newProdT.toFixed(2));
       $("#productoGet").html(prodSpan +" - Costo: "+produ);
       $('.modal-cost').modal('toggle');
   }
   ...
   function disparador(importacion) {
       Swal.fire({
           title: "¿Desea aprobar la importación?",
           icon: 'question',
           ...
           if (result.isConfirmed) {
               var url = base_url + "index.php/ventas/importacion/aprobar_import";
               $.ajax({
                   url: url,
                   type: "POST",
                   data: {codigo: importacion},
                   dataType: "json",
               beforeSend: function (data) {
               },
               success: function (data) {
